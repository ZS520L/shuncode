...
describe("parseMentions", () => {
	...
	describe("File mentions", () => {
		it("should handle simple file mention", async () => {
			...
			const expectedOutput = `<file_content path="src/index.ts">
console.log('Hello World');
</file_content>

Check 'src/index.ts' (see above for file content) for details`
		...
		it("should handle quoted file paths with spaces", async () => {
			...
			const expectedOutput = `<file_content path="path with spaces/file.txt">
console.log('Hello World');
</file_content>

Open 'path with spaces/file.txt' (see above for file content)`
		...
		it("should handle binary files", async () => {
			...
			const expectedOutput = `<file_content path="image.png">
(Binary file, unable to display content)
</file_content>

Check 'image.png' (see above for file content)`
		...
		it("should handle file read errors", async () => {
			...
			const expectedOutput = `<file_content path="missing.txt">
Error fetching content: Failed to access path "missing.txt": ENOENT: no such file or directory
</file_content>

Check 'missing.txt' (see above for file content)`
	...
	describe("Folder mentions", () => {
		it("should handle folder mention", async () => {
			...
			const expectedOutput = `<folder_content path="src/">
├── index.ts
├── utils/
└── README.md
...
Look in 'src/' (see above for folder content) folder`
	...
	describe("URL mentions", () => {
		it("should handle URL mention", async () => {
			...
			const expectedOutput = `<url_content url="https://example.com">
# Example Website

Content here
</url_content>

Visit 'https://example.com' (see above for site content) for info`
...
Visit 'https://example.com' (see above for site content)`
...
Visit 'https://example.com' (see above for site content)`
	...
	describe("Special mentions", () => {
		it("should handle @terminal mention", async () => {
			...
			const expectedOutput = `<terminal_output>
$ npm test
All tests passed!
</terminal_output>

See Terminal Output (see above for output) output`
		...
		it("should handle @git-changes mention", async () => {
			...
			const expectedOutput = `<git_working_state>
M  src/index.ts
A  src/new-file.ts
</git_working_state>

Review Working directory changes (see above for details)`
		...
		it("should handle git commit hash mention", async () => {
			...
			const expectedOutput = `<git_commit hash="abcdef1234567890">
...
See commit Git commit 'abcdef1234567890' (see above for commit info)`
	...
	describe("Multiple mentions", () => {
		it("should handle multiple mentions in order", async () => {
			...
			const expectedOutput = `<file_content path="file1.txt">
Content 1
</file_content>
...
Check 'file1.txt' (see above for file content) and 'file2.txt' (see above for file content)`
		...
		it("should handle duplicate mentions only once", async () => {
			...
			const expectedOutput = `<file_content path="file.txt">
Content
</file_content>

Check 'file.txt' (see above for file content) and again 'file.txt' (see above for file content)`
...
Check 'file.txt' (see above for file content), and 'https://example.com' (see above for site content)`
	...
	describe("Error handling", () => {
		it("should handle errors for each mention type gracefully", async () => {
			...
			const expectedOutput = `<file_content path="error.txt">
...
'error.txt' (see above for file content) Terminal Output (see above for output) Working directory changes (see above for details) Git commit 'abc1234567' (see above for commit info)`
...
Check 'file.txt' (see above for file content)!`
	...
	describe("Multiroot workspace mentions", () => {
		...
		it("should handle workspace-prefixed file mention", async () => {
			...
			const expectedOutput = `<file_content path="src/index.ts" workspace="frontend">
console.log('Frontend');
</file_content>

Check 'frontend:src/index.ts' (see above for file content)`
