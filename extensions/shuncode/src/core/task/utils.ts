...
import { ShuncodeApiReqCancelReason, ShuncodeApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const shuncodeMessages = params.messageStateHandler.getShuncodeMessages()
	const currentApiReqInfo: ShuncodeApiReqInfo = JSON.parse(shuncodeMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateShuncodeMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
