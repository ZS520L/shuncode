...
describe("Hook Executor", () => {
	...
	let mockMessages: ShuncodeMessage[]
...
`
	...
	beforeEach(async () => {
		...
		// Create .shuncoderules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".shuncoderules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.shuncoderules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getShuncodeMessages: () => messages,
				addToShuncodeMessages: async (msg: ShuncodeMessage) => {
					messages.push(msg)
				},
				updateShuncodeMessage: async (index: number, updates: Partial<ShuncodeMessage>) => {
					if (messages[index]) {
