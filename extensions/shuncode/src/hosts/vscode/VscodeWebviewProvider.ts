...
export class VscodeWebviewProvider extends WebviewProvider implements vscode.WebviewViewProvider {
	...
	public override updateSessionTabs(tabs: Array<{ id: string; title: string; state?: string }>, currentId?: string): void {
		// Call workbench command to update native title bar session tabs
		vscode.commands.executeCommand("shuncode.internal.updateSessionTabs", { tabs, currentId })
	}
	...
	public async resolveWebviewView(webviewView: vscode.WebviewView): Promise<void> {
		...
		webviewView.onDidChangeVisibility(
			async () => {
				if (this.webview?.visible) {
					await this.controller.ensureNewChatSession()
		...
		vscode.workspace.onDidChangeConfiguration(
			async (e) => {
				if (e && e.affectsConfiguration("shuncode.mcpMarketplace.enabled")) {
					// Update state when marketplace tab setting changes
					await this.controller.postStateToWebview()
				}
				if (e && e.affectsConfiguration("shuncode.fastContext")) {
					// Update state when Fast Context settings change
					await this.controller.postStateToWebview()
				}
			},
			null,
	...
	async handleWebviewMessage(message: WebviewMessage) {
		...
		switch (message.type) {
			...
			case "updateFastContextConfig": {
				// [SHUNCODE] Update Fast Context configuration from webview
				if (message.fastContextConfigUpdate) {
					const { key, value } = message.fastContextConfigUpdate
					try {
						const cfg = vscode.workspace.getConfiguration("shuncode.fastContext")
						await cfg.update(key, value, vscode.ConfigurationTarget.Global)
					} catch (error) {
						Logger.error(`[FastContext] Failed to update config ${key}:`, error)
					}
				}
				break
			}
	...
	/**
	 * [SHUNCODE-SHUNCODE] Send a lightweight message directly to the webview, bypassing gRPC state.
