 ...
 * ==================================
 *
 * This script generates Go code for the CLI version of Shuncode by extracting provider
...
//   4. Include universal fields (requestTimeoutMs, ulid, shuncodeAccountId) for all providers
//
...
func getFieldsByProvider(providerID string, allFields []ConfigField, required bool) []ConfigField {
	...
	for _, field := range allFields {
		...
		if override, hasOverride := GetFieldOverride(providerID, field.Name); hasOverride {
			...
			universalFields := []string{"requesttimeoutms", "shuncodeaccountid"}
