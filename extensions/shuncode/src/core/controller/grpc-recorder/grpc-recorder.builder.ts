	...
	static getRecorder(controller: Controller): IRecorder {
		if (!GrpcRecorderBuilder.recorder) {
			GrpcRecorderBuilder.recorder = GrpcRecorder.builder()
				.enableIf(process.env.GRPC_RECORDER_ENABLED === "true" && process.env.SHUNCODE_ENVIRONMENT === "local")
...
function testFilters(): GrpcRequestFilter[] {
	...
	return [
		...
		(req) => ["shuncode.UiService", "shuncode.McpService", "shuncode.WebService"].includes(req.service),
		(req) =>
			[
