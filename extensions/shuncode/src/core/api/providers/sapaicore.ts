...
namespace Bedrock {
	 ...
	 */
	export function formatMessagesForConverseAPI(messages: ShuncodeStorageMessage[]): BedrockMessage[] {
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithOrchestration(systemPrompt: string, messages: ShuncodeStorageMessage[]): ApiStream {
		try {
			...
			// sent directly to the LLM with the Shuncode system prompt without validation errors.
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: ShuncodeStorageMessage[]): ApiStream {
		const token = await this.getToken()
		const headers = {
			Authorization: `Bearer ${token}`,
			"AI-Resource-Group": this.options.sapAiResourceGroup || "default",
	...
	private convertMessageParamToSAPMessages(messages: ShuncodeStorageMessage[]): ChatMessage[] {
