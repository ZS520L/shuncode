...
export async function updateSettingsCli(controller: Controller, request: UpdateSettingsRequestCli): Promise<Empty> {
	...
	try {
		if (request.environment !== undefined) {
			ShuncodeEnv.setEnvironment(request.environment)
		...
		if (request.settings) {
			...
			// Update Shuncode web tools setting (requires telemetry)
			if (shuncodeWebToolsEnabled !== undefined) {
				if (controller.task) {
					telemetryService.captureShuncodeWebToolsToggle(controller.task.ulid, shuncodeWebToolsEnabled)
				}
				controller.stateManager.setGlobalState("shuncodeWebToolsEnabled", shuncodeWebToolsEnabled)
			}
			...
			if (focusChainSettings !== undefined) {
				...
				const newFocusChainSettings = {
					enabled: isEnabled,
					remindShuncodeInterval: focusChainSettings.remindShuncodeInterval,
				}
