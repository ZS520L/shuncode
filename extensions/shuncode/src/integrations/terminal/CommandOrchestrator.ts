...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const shuncodeMessages = callbacks.getShuncodeMessages()
		const lastCommandIndex = findLastIndex(shuncodeMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateShuncodeMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using ShuncodeTempManager for proper cleanup
		largeOutputLogPath = ShuncodeTempManager.createTempFilePath("large-output")
