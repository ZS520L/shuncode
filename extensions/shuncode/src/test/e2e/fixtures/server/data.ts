...
export class ShuncodeDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = ShuncodeDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof ShuncodeDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || ShuncodeDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = ShuncodeDataMock.findUserByToken(token)
