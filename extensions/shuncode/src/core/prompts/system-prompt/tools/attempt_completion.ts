...
const id = ShuncodeDefaultTool.ATTEMPT
...
IMPORTANT NOTE: This tool CANNOT be used until you've confirmed from the user that any previous tool uses were successful. Failure to do so will result in code corruption and system failure. Before using this tool, you must ask yourself in <thinking></thinking> tags if you've confirmed from the user that any previous tool uses were successful. If not, then DO NOT use this tool.`,
	parameters: [
		...
		},
		{
			...
			required: false,
			instruction:
				"A CLI command to execute to show a live demo of the result to the user. For example, use \`start localhost:3000\` to start a locally running development server. ONLY use this for launching servers, opening web URLs, or running executables. Do NOT use \`open\`, \`cat\`, \`echo\`, or any command that merely opens a file or prints text — the user can already see edited files in the editor. If the task was just editing files, do NOT provide a command at all. Do NOT provide a command that was already executed during this task — it will not be run again.",
...
const NATIVE_NEXT_GEN: ShuncodeToolSpec = {
	...
	parameters: [
		...
		},
		{
			...
			required: false,
			instruction:
				"An actionable terminal command that is non-verbose that allows user to review the result of your work. For example, use \`start localhost:3000\` to start a locally running development server. Commands like \`echo\` or \`cat\` that merely print text or open a file are not allowed. Do NOT provide a command that was already executed during this task — it will not be run again. Ensure the command is properly formatted for user's OS and does not contain any harmful instructions",
		},
		{
			name: "task_progress",
			required: false,
