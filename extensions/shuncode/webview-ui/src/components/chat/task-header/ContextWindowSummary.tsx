...
const TokenUsageDetails = memo<TokenUsageInfoProps>(({ tokensIn, tokensOut, cacheWrites, cacheReads }) => {
	const { t } = useI18n()
	const tokenDetailsConfig: Omit<TokenDetail, "value">[] = [
		{ title: t("taskHeader.promptTokens"), icon: "codicon-arrow-up" },
		{ title: t("taskHeader.completionTokens"), icon: "codicon-arrow-down" },
		{ title: t("taskHeader.cacheWrites"), icon: "codicon-arrow-left" },
		{ title: t("taskHeader.cacheReads"), icon: "codicon-arrow-right" },
	]
	const contextTokenDetails = useMemo(() => {
		const values = [tokensIn, tokensOut, cacheWrites || 0, cacheReads || 0]
		return tokenDetailsConfig.map((config, index) => ({ ...config, value: values[index] })).filter((item) => item.value)
	}, [tokensIn, tokensOut, cacheWrites, cacheReads, tokenDetailsConfig])
		...
		<div className="context-window-tooltip-content flex flex-col gap-2 bg-menu rounded shadow-sm z-100 w-60 p-1">
			{autoCompactThreshold > 0 && (
				<AccordionItem
					...
					<div className="space-y-1">
						<p className="text-xs leading-relaxed text-white">{t("taskHeader.clickContextWindowToSetThreshold")}</p>
						<p className="text-xs leading-relaxed mt-0 mb-0">{t("taskHeader.thresholdAutoCondenseDescription")}</p>
					</div>
				</AccordionItem>
			)}
