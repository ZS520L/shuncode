...
export class ShuncodeError extends Error {
	readonly title = "ShuncodeError"
	readonly _error: ErrorDetails

	// Error details per providers:
	// Shuncode: error?.error
	// Ollama: error?.cause
	// tbc
	constructor(
		raw: any,
	...
	/**
	 * Parses a stringified error into a ShuncodeError instance.
	 */
	static parse(errorStr?: string, modelId?: string): ShuncodeError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return ShuncodeError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a ShuncodeError instance.
	 * Always returns a ShuncodeError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): ShuncodeError {
		try {
			// If already a ShuncodeError, return it directly to prevent infinite recursion
			if (error instanceof ShuncodeError) {
				return error
			}
			return new ShuncodeError(JSON.parse(error), modelId, providerId)
		} catch {
			return new ShuncodeError(error, modelId, providerId)
		}
	}

	public isErrorType(type: ShuncodeErrorType): boolean {
		return ShuncodeError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: ShuncodeError): ShuncodeErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return ShuncodeErrorType.Balance
		}
		...
		if (message) {
			...
			if (message?.includes(SHUNCODE_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return ShuncodeErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return ShuncodeErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = ShuncodeErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = ShuncodeErrorType.Auth
	}
}
