...
/**
 * Interactive Playwright launcher for the Shuncode VS Code extension.
 *
 * Overview:
 *  - Starts the mock Shuncode API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Shuncode extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Shuncode sidebar.
 ...
 *
 *   3. VS Code will launch with the Shuncode extension loaded and gRPC recording enabled.
 *
...
import { ShuncodeApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with Shuncode extension is now running!")
