	...
	return (
		<div
			...
			<div
				...
				aria-label={t("chat.slash.commands")}
				...
				{filteredCommands.length > 0 ? (
					<>
						{renderCommandSection(defaultCommands, t("chat.slash.defaultCommands"), 0, true)}
						{renderCommandSection(workflowCommands, t("chat.slash.workflowCommands"), defaultCommands.length, false)}
					</>
				) : (
					<div aria-selected="false" className="py-2 px-3 cursor-default flex flex-col" role="option">
						<div className="text-[0.85em] text-(--vscode-descriptionForeground)">
							{t("chat.slash.noMatchingCommands")}
						</div>
					</div>
				)}
			</div>
