...
export default class LanguageProvider extends Disposable {
	...
	constructor(
		...
		const validateConfig = this._register(new UnifiedConfigValue<boolean>('validate.enabled', false, { scope, fallbackSection: this.id, fallbackSubSectionNameOverride: 'validate.enable' }));
