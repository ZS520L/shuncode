...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const shuncodeSettings = remoteConfig.providerSettings?.Shuncode
	if (shuncodeSettings) {
		transformed.planModeApiProvider = "shuncode"
		transformed.actModeApiProvider = "shuncode"
		providers.push("shuncode")
	}
...
export function clearRemoteConfig() {
	try {
		...
		// the remote config shuncode rules toggle state is stored in global state
