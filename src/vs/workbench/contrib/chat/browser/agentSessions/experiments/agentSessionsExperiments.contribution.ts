...
// [SHUNCODE] Dedicated action with the chat-sparkle icon that toggles the
// ShunCode chat panel (secondary side bar). We use our own command id so the
// chat-sparkle icon is preserved (referencing toggleAuxiliaryBar directly would
// inherit that command's own layout icon instead).
const SHUNCODE_TOGGLE_CHAT_ACTION_ID = 'shuncode.toggleChatPanel';

registerAction2(class ToggleShunCodeChatAction extends Action2 {
	constructor() {
		super({
			id: SHUNCODE_TOGGLE_CHAT_ACTION_ID,
			title: localize2('toggleShunCodeChat', "Toggle ShunCode Chat"),
	...
	run(accessor: ServicesAccessor): void {
		accessor.get(ICommandService).executeCommand('workbench.action.toggleAuxiliaryBar');
	}
});

// Register the button in the command center (to the right of the search box).
MenuRegistry.appendMenuItem(MenuId.CommandCenter, {
	command: {
		id: SHUNCODE_TOGGLE_CHAT_ACTION_ID,
		title: localize('toggleShunCodeChat', "Toggle ShunCode Chat"),
		icon: Codicon.chatSparkle,
	},
	icon: Codicon.chatSparkle,
	order: 10002
...
// Add to the global title bar if the command center is disabled.
