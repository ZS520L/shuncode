...
export class MultiRootCheckpointManager implements ICheckpointManager {
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		try {
			if (!this.enableCheckpoints || !this.initialized) {
				// Менеджер чекпоинтов не инициализирован
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
					message: t("checkpoint.error.notInitialized"),
				})
				return
			}
			...
			const primaryRoot = this.workspaceManager.getPrimaryRoot()
			if (!primaryRoot) {
				// Не задан основной корень рабочей области
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
					message: t("checkpoint.error.noPrimaryRoot"),
				})
				return
			}
			...
			const tracker = this.trackers.get(primaryRoot.path)
			if (!tracker) {
				// Нет трекера для основного корня
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
					message: t("checkpoint.error.noTrackerForPrimary"),
				})
				return
			}
			...
			// Не удалось показать diff
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: t("checkpoint.error.presentDiffFailed", { error: errorMessage }),
			})
		}
	}
}
