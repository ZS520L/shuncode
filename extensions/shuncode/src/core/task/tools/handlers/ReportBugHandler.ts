...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ShuncodeDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Shuncode wants to create a github issue...",
				message: `Shuncode is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const shuncodeVersion = ExtensionRegistryInfo.version
		...
		const bugReportData = JSON.stringify({
			...
			shuncode_version: shuncodeVersion,
		})
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("shuncode-version", shuncodeVersion)
				...
				await createAndOpenGitHubIssue("shuncode", "shuncode", "bug_report.yml", params)
			} catch (error) {
