...
export class Win32UpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected doCheckForUpdates(explicit: boolean, pendingCommit?: string): void {
		...
		const updatePromise = this.hasGitHubUpdateConfiguration()
			? this.getGitHubReleaseUpdate(this.getGitHubUpdatePlatform())
			: (() => {
				...
				return this.requestService.request({ url, headers }, CancellationToken.None).then<IUpdate | null>(asJson);
			})();
	...
	protected override getGitHubUpdatePlatform(): string {
		let platform = `win32-${process.arch}`;
		if (getUpdateType() === UpdateType.Archive) {
			platform += '-archive';
		} else if (this.productService.target === 'user') {
