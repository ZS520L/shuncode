...
/**
 * All valid hook types that can be created and executed by Shuncode.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Shuncode", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".shuncoderules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".shuncoderules", "hooks")
}
