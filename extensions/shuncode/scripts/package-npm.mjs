...
/**
 * NPM Package Builder for Shuncode CLI
 *
 * This script builds the Shuncode CLI NPM package (dist-standalone/).
...
async function main() {
	console.log("🚀 Building Shuncode NPM Package\n")
...
/**
 * Copy CLI binaries (shuncode and shuncode-host) for all platforms
...
async function copyCliBinaries() {
	...
	for (const { os, arch } of platforms) {
		...
		// Copy shuncode binary
		const shuncodeSource = path.join(CLI_BINARIES_DIR, `shuncode-${platformSuffix}`)
		const shuncodeDest = path.join(binDir, `shuncode-${platformSuffix}`)

		if (!fs.existsSync(shuncodeSource)) {
			console.error(`Error: CLI binary not found at ${shuncodeSource}`)
			console.error(`Please run: npm run compile-cli-all-platforms`)
			process.exit(1)
		}

		await cpr(shuncodeSource, shuncodeDest)
		fs.chmodSync(shuncodeDest, 0o755)
		console.log(`✓ shuncode-${platformSuffix} copied`)

		// Copy shuncode-host binary
		const hostSource = path.join(CLI_BINARIES_DIR, `shuncode-host-${platformSuffix}`)
		const hostDest = path.join(binDir, `shuncode-host-${platformSuffix}`)
		...
		console.log(`✓ shuncode-host-${platformSuffix} copied`)
	}
 ...
 * Ripgrep is needed by shuncode-core for file searching
...
async function createNpmPackageFiles() {
	...
	const manPageSource = path.join("cli", "man", "shuncode.1")
	const manDir = path.join(BUILD_DIR, "man")
	const manPageDest = path.join(manDir, "shuncode.1")
...
function setupBinaries() {
	...
	console.log(\`Setting up Shuncode CLI for \${cliPlatformSuffix}...\`);

	// Setup CLI binaries
	const binDir = path.join(__dirname, 'bin');
	
	// Check if platform-specific binaries exist
	const shuncodeSource = path.join(binDir, \`shuncode-\${cliPlatformSuffix}\`);
	const shuncodeHostSource = path.join(binDir, \`shuncode-host-\${cliPlatformSuffix}\`);
	
	if (!fs.existsSync(shuncodeSource)) {
		console.error(\`Error: Binary not found for platform \${cliPlatformSuffix}\`);
		console.error(\`Expected: \${shuncodeSource}\`);
		console.error(\`Supported platforms: darwin-arm64, darwin-amd64, linux-amd64, linux-arm64\`);
		process.exit(1);
	}
	
	if (!fs.existsSync(shuncodeHostSource)) {
		console.error(\`Error: Binary not found for platform \${cliPlatformSuffix}\`);
		console.error(\`Expected: \${shuncodeHostSource}\`);
		process.exit(1);
	}

	// Create symlinks or copies to the generic names
	const shuncodeTarget = path.join(binDir, 'shuncode');
	const shuncodeHostTarget = path.join(binDir, 'shuncode-host');
	
	// Remove existing files if they exist
	[shuncodeTarget, shuncodeHostTarget].forEach(target => {
	...
	if (platform === 'win32') {
		// Windows: copy files
		fs.copyFileSync(shuncodeSource, shuncodeTarget);
		fs.copyFileSync(shuncodeHostSource, shuncodeHostTarget);
		console.log('✓ Copied platform-specific CLI binaries');
	} else {
		// Unix: create symlinks
		fs.symlinkSync(path.basename(shuncodeSource), shuncodeTarget);
		fs.symlinkSync(path.basename(shuncodeHostSource), shuncodeHostTarget);
		console.log('✓ Created symlinks to platform-specific CLI binaries');
		
		// Make binaries executable
		try {
			fs.chmodSync(shuncodeSource, 0o755);
			fs.chmodSync(shuncodeHostSource, 0o755);
			fs.chmodSync(shuncodeTarget, 0o755);
			fs.chmodSync(shuncodeHostTarget, 0o755);
		} catch (error) {
	...
	// Copy ripgrep binary to root (where shuncode-core expects it)
	...
	console.log('✓ Shuncode CLI installation complete');
	console.log('');
	console.log('Usage:');
	console.log('  shuncode        - Start Shuncode CLI');
	console.log('  shuncode-host   - Start Shuncode host service');
	console.log('');
