...
import { ShuncodeEnv, EnvironmentConfig } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicShuncodeHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/ShuncodeError"
import { telemetryService } from "@/services/telemetry"
import { SHUNCODE_API_ENDPOINT } from "@/shared/shuncode/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type ShuncodeAccountUserInfo, type ShuncodeAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface ShuncodeAuthApiUser {
	subject: string | null
	email: string
	name: string
	shuncodeUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface ShuncodeAuthResponseData {
	/**
...
export interface ShuncodeAuthApiTokenExchangeResponse {
	success: boolean
	data: ShuncodeAuthResponseData
}

export interface ShuncodeAuthApiTokenRefreshResponse {
	success: boolean
	data: ShuncodeAuthResponseData
}

export class ShuncodeAuthProvider {
	readonly name = "shuncode"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: ShuncodeAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("shuncode:shuncodeAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: ShuncodeAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Shuncode auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<ShuncodeAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveShuncodeAuthInfo(controller: Controller): Promise<ShuncodeAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("shuncode:shuncodeAccountId")
			...
			let storedAuthData: ShuncodeAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("shuncodeAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("shuncode:shuncodeAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Shuncode, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<ShuncodeAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: ShuncodeAuthInfo): Promise<ShuncodeAuthInfo> {
		try {
			const endpoint = new URL(SHUNCODE_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: ShuncodeAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(SHUNCODE_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<ShuncodeAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(SHUNCODE_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: ShuncodeAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("shuncode:shuncodeAccountId", JSON.stringify(shuncodeAuthInfo))

			return shuncodeAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: ShuncodeAuthApiTokenExchangeResponse["data"]): Promise<ShuncodeAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${ShuncodeEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.shuncodeUserId || "",
	...
	private async headers() {
		return {
			...
			...(await buildBasicShuncodeHeaders()),
		}
	}
}
