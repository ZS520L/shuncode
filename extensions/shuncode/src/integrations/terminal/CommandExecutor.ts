 ...
 *
 * IMPORTANT: Subagent commands (shuncode CLI) are ALWAYS routed to use
...
import { isSubagentCommand, transformShuncodeCommand } from "@integrations/cli-subagents/subagent_command"
import { telemetryService } from "@services/telemetry"
import { findLastIndex } from "@shared/array"
import { ShuncodeToolResponseContent } from "@shared/messages"
...
export class CommandExecutor {
	 ...
	 *
	 * Routing logic:
	 * 1. Subagent commands (shuncode CLI) → Always use StandaloneTerminalManager
	 *    This ensures subagents run in hidden terminals, not cluttering the user's VSCode terminal
	 ...
	 */
	async execute(command: string, timeoutSeconds: number | undefined): Promise<[boolean, ShuncodeToolResponseContent]> {
		// Transform subagent commands to ensure flags are correct
		const isSubagent = isSubagentCommand(command)
		if (isSubagent) {
			command = transformShuncodeCommand(command)
		}
	...
	async cancelBackgroundCommand(): Promise<boolean> {
		...
		if (cancelled) {
			...
			const messages = this.callbacks.getShuncodeMessages()
			...
			if (lastCommandOutputIndex !== -1) {
				...
				await this.callbacks.updateShuncodeMessage(lastCommandOutputIndex, {
	...
	/**
	 * Set the interactive responder configuration for auto-responding to terminal prompts.
	 * @param config The responder configuration, or undefined to disable
	 */
	setInteractiveResponderConfig(config: import("./TerminalInteractiveResponder").InteractiveResponderConfig | undefined): void {
		this.standaloneManager.setInteractiveResponderConfig(config)
	}
