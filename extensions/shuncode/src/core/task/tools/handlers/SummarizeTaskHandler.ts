...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ShuncodeDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						shuncodeMessages: config.messageState.getShuncodeMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .shuncodeignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(ShuncodeDefaultTool.FILE_READ, relPath)) {
						try {
			...
			if (fileContents) {
				const fileMentionString = loadedFilePaths.map((path) => `'${path}'`).join(", ") + " (see above for file content)"
				fileContents =
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getShuncodeMessages(),
				config.api,
