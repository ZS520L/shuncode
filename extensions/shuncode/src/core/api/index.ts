...
const OPENAI_REASONING_EFFORT_PROVIDERS = new Set<ApiProvider>([
	"openai",
	"openai-native",
	"openai-codex",
	"sapaicore",
...
export function applyOpenAiReasoningEffort(
	...
	mode: Mode,
	openaiReasoningEffort: ApiConfiguration["planModeReasoningEffort"],
): ApiConfiguration {
	...
	const currentProvider = apiMode === "plan" ? configuration.planModeApiProvider : configuration.actModeApiProvider

	if (!currentProvider || !OPENAI_REASONING_EFFORT_PROVIDERS.has(currentProvider)) {
		return configuration
	}

	return apiMode === "plan"
		? { ...configuration, planModeReasoningEffort: openaiReasoningEffort }
		: { ...configuration, actModeReasoningEffort: openaiReasoningEffort }
}
