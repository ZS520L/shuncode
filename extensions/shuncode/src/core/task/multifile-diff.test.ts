...
describe("multifile-diff", () => {
	...
	describe("showChangedFilesDiff", () => {
		...
		const mockMessages: ShuncodeMessage[] = [
			{
				ts: mockMessageTs,
				type: "say",
		...
		beforeEach(() => {
			messageStateHandlerStub.getShuncodeMessages.returns(mockMessages)
		})
		...
		it("should successfully show diff for changes since last task completion", async () => {
			...
			const messagesWithCompletion: ShuncodeMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getShuncodeMessages.returns(messagesWithCompletion)
		...
		it("should handle message not found error", async () => {
			// Arrange
			messageStateHandlerStub.getShuncodeMessages.returns([])
		...
		it("should handle missing checkpoint hash", async () => {
			// Arrange
			const messagesWithoutHash: ShuncodeMessage[] = [
				{
					ts: mockMessageTs,
					type: "say",
					say: "text",
			...
			messageStateHandlerStub.getShuncodeMessages.returns(messagesWithoutHash)
		...
		it("should use first checkpoint when no last completion found", async () => {
			// Arrange
			const messagesWithFirstCheckpoint: ShuncodeMessage[] = [
				{
					ts: 1234567000,
					type: "say",
			...
			messageStateHandlerStub.getShuncodeMessages.returns(messagesWithFirstCheckpoint)
