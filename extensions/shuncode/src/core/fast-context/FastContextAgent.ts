...
RULES:
- Use the available tools (grep, read_file, find_files) to locate relevant code
	...
	async search(query: string, onProgress?: ProgressCallback, options?: { scope?: string; directoryHints?: string }): Promise<FastContextResult> {
		...
		try {
			for (let turn = 0; turn < this.config.maxTurns; turn++) {
				...
				const inputs: ToolCallInput[] = response.tool_calls.map((tc: any) => {
				...
				const operations = inputs.map((inp) => {
					...
					return {
						type: inp.tool as "grep" | "read_file" | "find_files",
						args: argsStr,
				...
				messages.push({ role: "assistant", content: response.content || undefined, tool_calls: response.tool_calls })
