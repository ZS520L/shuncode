...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("shuncode.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("shuncode.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): ShuncodeMessage[] {
	...
	const messages: ShuncodeMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Shuncode.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
