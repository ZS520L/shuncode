...
// Featured models for Shuncode provider organized by tabs
...
const FREE_SHUNCODE_MODELS = freeModels.map((m) => m.id)
...
const OpenRouterModelPicker: React.FC<OpenRouterModelPickerProps> = ({ isPopup, currentMode, showProviderRouting }) => {
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		const isShuncode = selected.selectedProvider === "shuncode"
		// Makes sure "Free" featured models have $0 pricing for Shuncode provider
		if (isShuncode && FREE_SHUNCODE_MODELS.includes(selected.selectedModelId)) {
			return {
				...selected,
				selectedModelInfo: {
	...
	return (
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				...
				{modeFields.apiProvider === "shuncode" && (
					<>
						{/* Tabs */}
			...
			{hasInfo ? (
				...
				<p
					...
					If you're unsure which model to choose, Shuncode works best with{" "}
					<VSCodeLink
