...
export class AnthropicHandler implements ApiHandler {
	...
	@withRetry()
	async *createMessage(systemPrompt: string, messages: ShuncodeStorageMessage[], tools?: AnthropicTool[]): ApiStream {
		...
		for await (const chunk of stream) {
			switch (chunk?.type) {
				...
				case "content_block_stop":
					// For tools with no parameters (e.g. evaluate_task), Anthropic may not send
					// any input_json_delta events. Yield a final tool_calls chunk so the tool
					// call is registered by processNativeToolCalls.
					if (lastStartedToolCall.id && lastStartedToolCall.name && !toolCallHadInput) {
						yield {
							type: "tool_calls",
							tool_call: {
								...lastStartedToolCall,
	...
	getModel(): { id: string; info: ModelInfo } {
		...
		// Support custom model IDs not in the static map (e.g. from multi-provider configs)
		if (modelId) {
			// Infer capabilities from model name patterns
			const isThinkingModel = modelId.includes("thinking") || modelId.includes("think")
			...
			return {
				...
				info: {
					...
					inputPrice: isOpus ? 15.0 : isSonnet ? 3.0 : 3.0,
					outputPrice: isOpus ? 75.0 : isSonnet ? 15.0 : 15.0,
					cacheWritesPrice: isOpus ? 18.75 : isSonnet ? 3.75 : 3.75,
					cacheReadsPrice: isOpus ? 1.5 : isSonnet ? 0.3 : 0.3,
				},
			}
		}
		return {
