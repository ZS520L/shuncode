...
const ConfirmationDialog = memo<{
	...
	<div className="text-sm my-2 flex items-center gap-0 justify-between">
		<span className="font-semibold text-sm">{t("taskHeader.compactCurrentTaskQuestion")}</span>
		<span className="flex gap-1">
			<VSCodeButton
				appearance="secondary"
				className="text-sm"
				onClick={onCancel}
				title={t("taskHeader.noKeepTaskAsIs")}
				type="button">
				{t("taskHeader.cancel")}
			</VSCodeButton>
			<VSCodeButton
		...
		<div className="flex flex-col my-1.5" onMouseLeave={debounceCloseHover}>
			<div className="flex gap-1 flex-row @max-xs:flex-col @max-xs:items-start items-center text-sm">
				<div className="flex items-center gap-1.5 flex-1 whitespace-nowrap">
					<span className="cursor-pointer text-sm" title={t("taskHeader.currentTokensUsedInRequest")}>
					...
					<div className="flex relative items-center gap-1 flex-1 w-full h-full" onMouseEnter={() => setIsOpened(true)}>
						<HoverCard>
							...
							<HoverCardTrigger asChild>
								<div
									aria-label={t("taskHeader.autoCondenseThreshold")}
									aria-valuemax={100}
									aria-valuemin={0}
									aria-valuenow={Math.round(threshold * 100)}
									aria-valuetext={`${Math.round(threshold * 100)}% ${t("taskHeader.threshold")}`}
									...
									<Progress
										aria-label={t("taskHeader.contextWindowUsageProgress")}
										color="success"
					...
					<span className="cursor-pointer text-sm" title={t("taskHeader.maxContextWindowForModel")}>
						{formatTokenNumber(tokenData.max)}
					</span>
				</div>
				<CompactTaskButton onClick={handleCompactClick} />
			</div>
			{confirmationNeeded && <ConfirmationDialog onCancel={handleCancel} onConfirm={handleConfirm} t={t} />}
		</div>
	)
}
