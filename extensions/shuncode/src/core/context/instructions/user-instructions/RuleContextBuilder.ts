...
type MessageStateHandlerLike = {
	getShuncodeMessages(): ShuncodeMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Shuncode Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in shuncodeMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const shuncodeMessages = deps.messageStateHandler.getShuncodeMessages()
		...
		const lastUserMsg = [...shuncodeMessages]
			.reverse()
		...
		// (3) Files edited by Shuncode during this task (completed operations):
