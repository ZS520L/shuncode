...
async function constructSession(
  ...
  // [SHUNCODE] GPU (DML) loading can hang indefinitely on some systems.
  // Try with configured providers (dml + cpu) first with a timeout,
  // then fall back to cpu-only if it hangs or throws.
  const GPU_LOAD_TIMEOUT_MS = 15000;

  // If executionProviders includes a GPU provider (dml/cuda), try with timeout
  const hasGpuProvider = executionProviders.some(
    (p) => p === "dml" || p === "cuda" || p === "tensorrt"
  );

  if (hasGpuProvider) {
    try {
      const sessionPromise = InferenceSession.create(buffer, {
        executionProviders,
      });
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error(`GPU model load timed out after ${GPU_LOAD_TIMEOUT_MS}ms — GPU provider may be incompatible`)),
          GPU_LOAD_TIMEOUT_MS,
        );
      });
      return await Promise.race([sessionPromise, timeoutPromise]);
    } catch (err) {
      console.warn("[SHUNCODE ONNX]", err.message || err);
      console.warn(
        "[SHUNCODE ONNX] GPU execution provider failed or timed out. Falling back to CPU-only.",
      );
      // Fall through to CPU-only fallback below
    }
  }

  // CPU-only fallback (always works with onnxruntime-node)
  try {
    return await InferenceSession.create(buffer, {
      executionProviders: ["cpu"],
    });
  } catch (cpuErr) {
    // Last resort: try wasm (for onnxruntime-web environments)
    console.warn("[SHUNCODE ONNX] CPU fallback failed:", cpuErr.message || cpuErr);
    return await InferenceSession.create(buffer, {
...
async function sessionRun(session, inputs) {
  ...
  // [SHUNCODE] Convert custom Tensor objects to real ORT Tensors.
  // In onnxruntime-node >=1.20, Tensor.data and Tensor.location are prototype getters,
  // not own properties. Our custom Tensor (created via Object.assign) loses these getters.
  // We must reconstruct proper ORT Tensors for session.run().
  const ortInputs = Object.create(null);
  for (const key of Object.keys(checkedInputs)) {
    const t = checkedInputs[key];
    if (t instanceof ONNXTensor) {
      ortInputs[key] = t;
    } else {
      // Reconstruct from own properties (cpuData/data + type + dims)
      const tensorData = t.cpuData ?? t.data;
      ortInputs[key] = new ONNXTensor(t.type, tensorData, t.dims);
    }
  }

  try {
    // @ts-ignore
    let output = await session.run(ortInputs);
    output = replaceTensors(output);
    return output;
  } catch (e) {
    // This usually occurs when the inputs are of the wrong type.
    console.error(`An error occurred during model execution: "${e}".`);
    console.error("Inputs given to model:", ortInputs);
    throw e;
  }
}
