...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: ShuncodeStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "shuncode-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: ShuncodeStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "shuncode-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user: string = "shuncode-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user: string = "shuncode-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user: string = "shuncode-user"): Promise<void> {
