import { BooleanRequest, EmptyRequest, StringArrayRequest } from "@shared/proto/shuncode/common"
import { GetTaskHistoryRequest, TaskFavoriteRequest } from "@shared/proto/shuncode/task"
...
const HistoryView = ({ onDone }: HistoryViewProps) => {
	...
	const loadTaskHistory = useCallback(async () => {
		try {
			...
			const taskHistoryById = new Map(taskHistory.map((item) => [item.id, item]))
			setTasks(
				(response.tasks || []).map((task) => ({
					...task,
					evaluation: taskHistoryById.get(task.id)?.evaluation,
				})),
			)
		} catch (error) {
