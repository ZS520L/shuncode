import type { Banner, BannerRules, BannersResponse } from "@shared/ShuncodeBanner"
import { BannerActionType, type BannerCardData } from "@shared/shuncode/banner"
...
export class BannerService {
	...
	private async getExtensionVersion(): Promise<string> {
		try {
			...
			return hostVersion.shuncodeVersion || ""
		} catch (error) {
	...
	private async getIdeType(): Promise<string> {
		try {
			...
			// Use shuncodeType field which contains values like "VSCode Extension", "Shuncode for JetBrains", "CLI", etc.
			const shuncodeType = hostVersion.shuncodeType?.toLowerCase() || ""

			if (shuncodeType.includes("vscode")) {
				return "vscode"
			}
			if (shuncodeType.includes("jetbrains")) {
				return "jetbrains"
			}
			if (shuncodeType.includes("cli")) {
				return "cli"
			}
	...
	/**
	 * Checks if the fresh-install suppression sentinel is present in dismissed banners
	 */
	private isFreshInstallSentinelPresent(): boolean {
		try {
			const dismissedBanners = this._controller.stateManager.getGlobalStateKey("dismissedBanners") || []
			return dismissedBanners.some((b) => b.bannerId === "__fresh_install_suppress_all__")
		} catch {
			return false
		}
	}
	...
	/**
	 * Removes the fresh-install suppression sentinel from dismissed banners
	 */
	private clearFreshInstallSentinel(): void {
		try {
			const dismissedBanners = this._controller.stateManager.getGlobalStateKey("dismissedBanners") || []
			const filtered = dismissedBanners.filter((b) => b.bannerId !== "__fresh_install_suppress_all__")
			this._controller.stateManager.setGlobalState("dismissedBanners", filtered)
		} catch {
			// Ignore
		}
	}
