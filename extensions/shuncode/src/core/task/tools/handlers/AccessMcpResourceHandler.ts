...
import { ShuncodeAsk, ShuncodeAskUseMcpServer } from "@shared/ExtensionMessage"
...
export class AccessMcpResourceHandler implements IFullyManagedTool {
	readonly name = ShuncodeDefaultTool.MCP_ACCESS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const partialMessage = JSON.stringify({
			...
			arguments: undefined,
		} satisfies ShuncodeAskUseMcpServer)
		...
		if (shouldAutoApprove) {
			...
			await uiHelpers.ask("use_mcp_server" as ShuncodeAsk, partialMessage, block.partial).catch(() => {})
		}
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!server_name) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(ShuncodeDefaultTool.MCP_ACCESS, "server_name")
		}

		if (!uri) {
			config.taskState.consecutiveMistakeCount++
			return await config.callbacks.sayAndCreateMissingParamError(ShuncodeDefaultTool.MCP_ACCESS, "uri")
		}
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Shuncode wants to access ${uri || "unknown resource"} on ${server_name || "unknown server"}`
