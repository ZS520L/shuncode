import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/shuncode/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Shuncode", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".shuncoderules", "hooks")
