...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".shuncode_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Shuncode Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "shuncode@example.com"], { cwd: workspacePath })
