...
export class SearchFilesToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// [P4] Validate regex syntax early to give clear error before expensive search
		try {
			new RegExp(regex)
		} catch (regexError) {
			config.taskState.consecutiveMistakeCount++
			return formatResponse.toolError(
				...
				`Tip: Escape special characters like . * + ? [ ] ( ) { } | \\ ^ $ with a backslash.`,
			)
		}
		...
		return truncateToolOutput(results)
	}
}
