import { AuthState, UserInfo } from "@shared/proto/shuncode/account"
import { type EmptyRequest, String } from "@shared/proto/shuncode/common"
...
import { AuthInvalidTokenError, AuthNetworkError } from "../error/ShuncodeError"
import { featureFlagsService } from "../feature-flags"
import { ShuncodeAuthProvider } from "./providers/ShuncodeAuthProvider"
...
export interface ShuncodeAuthInfo {
	/**
	 * accessToken
	 */
	idToken: string
	 ...
	 */
	expiresAt?: number
	userInfo: ShuncodeAccountUserInfo
	provider: string
	startedAt?: number
}

export interface ShuncodeAccountUserInfo {
	createdAt: string
	displayName: string
	email: string
	id: string
	organizations: ShuncodeAccountOrganization[]
	/**
	 * Shuncode app base URL, used for webview UI and other client-side operations
	 */
	appBaseUrl?: string
	/**
...
export interface ShuncodeAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
export class AuthService {
	...
	protected _shuncodeAuthInfo: ShuncodeAuthInfo | null = null
	protected _provider: ShuncodeAuthProvider
	...
	protected constructor(controller: Controller) {
		this._provider = new ShuncodeAuthProvider()
	...
	getActiveOrganizationId(): string | null {
		if (!this._shuncodeAuthInfo?.userInfo?.organizations) {
			return null
		}
		const activeOrg = this._shuncodeAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): ShuncodeAccountOrganization[] | undefined {
		return this._shuncodeAuthInfo?.userInfo?.organizations
	}

	private async internalGetAuthToken(provider: ShuncodeAuthProvider): Promise<string | null> {
		try {
			let shuncodeAccountAuthToken = this._shuncodeAuthInfo?.idToken
			if (!this._shuncodeAuthInfo || !shuncodeAccountAuthToken || this._shuncodeAuthInfo.provider !== provider.name) {
				// Not authenticated
				return null
			}

			// Check if token has expired
			if (await provider.shouldRefreshIdToken(shuncodeAccountAuthToken, this._shuncodeAuthInfo.expiresAt)) {
				...
				this._refreshPromise = (async () => {
					...
					try {
						const updatedAuthInfo = await provider.retrieveShuncodeAuthInfo(this._controller)
						if (updatedAuthInfo) {
							this._shuncodeAuthInfo = updatedAuthInfo
							this._authenticated = true
							shuncodeAccountAuthToken = updatedAuthInfo.idToken
					...
					return shuncodeAccountAuthToken
				})()

				shuncodeAccountAuthToken = await this._refreshPromise
			}

			return shuncodeAccountAuthToken || null
		} catch (error) {
	 ...
	 * @returns The provider name (e.g., "shuncode", "firebase"), or null if not authenticated
	 */
	getProviderName(): string | null {
		return this._shuncodeAuthInfo?.provider ?? null
	}
	...
	getInfo(): AuthState {
		...
		let user: any = null
		if (this._shuncodeAuthInfo && this._authenticated) {
			const userInfo = this._shuncodeAuthInfo.userInfo
			this._shuncodeAuthInfo.userInfo.appBaseUrl = ShuncodeEnv.config()?.appBaseUrl
	...
	async handleAuthCallback(authorizationCode: string, provider: string): Promise<void> {
		try {
			this._shuncodeAuthInfo = await this._provider.signIn(this._controller, authorizationCode, provider)
			this._authenticated = this._shuncodeAuthInfo?.idToken !== undefined
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		try {
			this._shuncodeAuthInfo = await this.retrieveAuthInfo()
			if (this._shuncodeAuthInfo) {
				this._authenticated = true
				await this.sendAuthStatusUpdate()
			} else {
	...
	private async retrieveAuthInfo(): Promise<ShuncodeAuthInfo | null> {
		// If a refresh is already in progress, wait for it to complete
		if (this._refreshPromise) {
			Logger.info("Token refresh already in progress, waiting for completion")
		...
		return this._provider.retrieveShuncodeAuthInfo(this._controller)
	}
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		if (this._shuncodeAuthInfo?.userInfo?.id) {
			telemetryService.identifyAccount(this._shuncodeAuthInfo.userInfo)
			// Poll feature flags immediately for authenticated users to ensure cache is populated
			await featureFlagsService.poll(this._shuncodeAuthInfo.userInfo?.id)
		} else {
	...
	private destroyTokens() {
		this._controller.stateManager.setSecret("shuncodeAccountId", undefined)
		this._controller.stateManager.setSecret("shuncode:shuncodeAccountId", undefined)
	}
}
