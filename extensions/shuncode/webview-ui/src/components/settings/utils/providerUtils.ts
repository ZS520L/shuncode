 ...
 * For providers with dynamic models (openrouter, shuncode, ollama, etc.), returns undefined.
...
export function normalizeApiConfiguration(
	...
	const getProviderData = (models: Record<string, ModelInfo>, defaultId: string) => {
		...
		if (modelId && modelId in models) {
			...
			// Custom model ID not in static map (e.g. from multi-provider config)
			// Keep the user-specified model ID instead of falling back to default
			selectedModelId = modelId
			selectedModelInfo = models[defaultId] // Use default info as fallback for display
		} else {
 ...
 * For Shuncode provider: excludes :free models (except Minimax models)
 * For OpenRouter/Vercel: excludes shuncode/ prefixed models
...
export function filterOpenRouterModelIds(modelIds: string[], provider: ApiProvider): string[] {
	if (provider === "shuncode") {
		// For Shuncode provider: exclude :free models, but keep Minimax models
	...
	// For OpenRouter and Vercel AI Gateway providers: exclude Shuncode-specific models
	return modelIds.filter((id) => !id.startsWith("shuncode/"))
}
