...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: ShuncodeAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to ShuncodeAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): ShuncodeAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: ShuncodeAssistantToolUseBlock["reasoning_details"]): ShuncodeAssistantToolUseBlock[] {
		const results: ShuncodeAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(SHUNCODE_MCP_TOOL_IDENTIFIER)) {
				const [key, toolName] = pending.name.split(SHUNCODE_MCP_TOOL_IDENTIFIER)
				results.push({
					type: "tool_use",
					name: ShuncodeDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as ShuncodeDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): ShuncodeAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): ShuncodeAssistantRedactedThinkingBlock[] {
