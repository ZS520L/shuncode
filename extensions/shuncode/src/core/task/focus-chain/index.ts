...
import { ShuncodeSay } from "../../../shared/ExtensionMessage"
...
export class FocusChainManager {
	...
	public shouldIncludeFocusChainInstructions(): boolean {
		...
		const reachedReminderInterval =
			this.taskState.apiRequestsSinceLastTodoUpdate >= this.focusChainSettings.remindShuncodeInterval
		// Include when a checklist already exists and needs updating
		const hasExistingChecklist = !!this.taskState.currentFocusChainChecklist
		// Include if no list has been created after several requests
