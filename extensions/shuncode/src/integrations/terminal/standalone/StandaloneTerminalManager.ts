...
import type { InteractiveResponderConfig } from "../TerminalInteractiveResponder"
...
export class StandaloneTerminalManager implements ITerminalManager {
	...
	/** Interactive responder config for auto-responding to prompts */
	private interactiveResponderConfig: InteractiveResponderConfig | undefined
	...
	runCommand(terminalInfo: TerminalInfo, command: string): TerminalProcessResultPromise {
		...
		const process = new StandaloneTerminalProcess(this.interactiveResponderConfig)
	...
	async getOrCreateTerminal(cwd: string): Promise<TerminalInfo> {
		...
		const newTerminalInfo = this.registry.createTerminal({
			cwd: cwd,
			name: `Shuncode Terminal ${this.registry.size + 1}`,
		})
	...
	getTerminalSnapshots(options: TerminalSnapshotOptions = {}): TerminalSnapshot[] {
		return Array.from(this.terminalIds)
			.map((id) => this.registry.getTerminal(id))
			.filter((t): t is TerminalInfo => t !== undefined && (options.terminalId === undefined || t.id === options.terminalId))
			.map((terminal) => {
				const process = this.processes.get(terminal.id)
				const output = process?.getOutput?.(options.lineLimit) ?? ""
				const outputLineCount = output ? output.split("\n").length : 0
				const status: TerminalSnapshot["status"] = terminal.busy ? "running" : "completed"
				return {
					id: terminal.id,
					busy: terminal.busy,
					isHot: process?.isHot ?? false,
					status,
					lastCommand: terminal.lastCommand,
					output,
					outputLineCount,
					outputTruncated: output.includes("lines truncated"),
				}
			})
	}
	...
	processOutput(outputLines: string[], overrideLimit?: number, isSubagentCommand?: boolean): string {
		...
		if (outputLines.length > limit) {
			// Smart truncation: tail-biased with error extraction from discarded middle
			const { smartTruncateOutput } = require("../smartTruncation")
			const { output } = smartTruncateOutput(outputLines, limit)
			return output
		}
	...
	/**
	 * Set the interactive responder configuration.
	 * When enabled, terminal processes will auto-respond to Y/N prompts.
	 * @param config The responder configuration, or undefined to disable
	 */
	setInteractiveResponderConfig(config: InteractiveResponderConfig | undefined): void {
		this.interactiveResponderConfig = config
	}
	...
	trackBackgroundCommand(
		...
		// Use ShuncodeTempManager for proper temp file management and cleanup
		const logFilePath = ShuncodeTempManager.createTempFilePath("background")
