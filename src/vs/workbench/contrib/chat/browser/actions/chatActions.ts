...
import { ChatViewId, ChatViewContainerId, IChatWidget, IChatWidgetService, isIChatViewViewContext } from '../chat.js';
...
export function registerChatActions() {
	...
	registerAction2(class ToggleChatAction extends Action2 {
		...
		async run(accessor: ServicesAccessor) {
			...
			// [SHUNCODE] Resolve effective view ID — fall back to first view in
			// the chat container when the built-in Copilot view is disabled.
			let effectiveViewId = ChatViewId;
			if (!viewDescriptorService.getViewLocationById(effectiveViewId)) {
				const container = viewDescriptorService.getViewContainerById(ChatViewContainerId);
				if (container) {
					const views = viewDescriptorService.getViewContainerModel(container).visibleViewDescriptors;
					if (views.length > 0) {
						effectiveViewId = views[0].id;
					}
				}
			}

			const chatLocation = viewDescriptorService.getViewLocationById(effectiveViewId);
			const chatVisible = viewsService.isViewVisible(effectiveViewId);
