...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Shuncode extension.
	 * this is specifically to avoid capturing errors from anything other than Shuncode
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Shuncode
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "shuncode"
			if (stacktrace?.value?.toLowerCase().includes("shuncode")) {
				return event
			}
			// Check stack frames for Shuncode extension path
			const frames = stacktrace?.frames
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					if (frames[j]?.filename?.includes("shuncode")) {
						return event
					}
				}
			}
