...
export class VscodeTerminalManager implements ITerminalManager {
	...
	async getOrCreateTerminal(cwd: string): Promise<ITerminalInfo> {
		...
		const matchingTerminal = terminals.find((t) => {
			...
			const terminalCwd = t.terminal.shellIntegration?.cwd // one of shuncode's commands could have changed the cwd of the terminal
			if (!terminalCwd) {
	...
	getTerminalSnapshots(options: TerminalSnapshotOptions = {}): TerminalSnapshot[] {
		return Array.from(this.terminalIds)
			.map((id) => TerminalRegistry.getTerminal(id))
			.filter(
				(t): t is TerminalInfo => t !== undefined && (options.terminalId === undefined || t.id === options.terminalId),
			)
			.map((terminal) => {
