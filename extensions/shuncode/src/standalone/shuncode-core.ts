...
async function main() {
	log("\n\n\nStarting shuncode-core service...\n\n\n")
...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// shuncode-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the shuncode-core process by:
...
function showHelp() {
	Logger.log(`
Shuncode Core - Standalone Server

Usage: node shuncode-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Shuncode data storage (default: ~/.shuncode)
