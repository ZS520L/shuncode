...
import type { ShuncodeSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified ShuncodeSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: ShuncodeSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: ShuncodeSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ShuncodeDefaultTool.GENERATE_EXPLANATION
