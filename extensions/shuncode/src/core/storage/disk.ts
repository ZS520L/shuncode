...
import type { TaskEvaluation } from "@core/task/evaluation"
...
export const GlobalFileNames = {
	...
	mcpSettings: "shuncode_mcp_settings.json",
	shuncodeRules: ".shuncoderules",
	workflows: ".shuncoderules/workflows",
	hooksDir: ".shuncoderules/hooks",
	shuncoderuleSkillsDir: ".shuncoderules/skills",
	shuncodeSkillsDir: ".shuncode/skills",
	...
	taskEvaluation: "evaluation.json",
...
/**
 * Returns the cross-platform path to the Shuncode home directory (~/.shuncode).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.shuncode
 * - Linux: /home/username/.shuncode
 * - Windows: C:\Users\username\.shuncode
 *
 * This is intended to eventually replace ~/Documents/Shuncode as the global config location.
 */
export function getShuncodeHomePath(): string {
	return path.join(os.homedir(), ".shuncode")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const shuncodeRulesDir = path.join(userDocumentsPath, "Shuncode", "Rules")
	try {
		await fs.mkdir(shuncodeRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Shuncode", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return shuncodeRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const shuncodeWorkflowsDir = path.join(userDocumentsPath, "Shuncode", "Workflows")
	try {
		await fs.mkdir(shuncodeWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Shuncode", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return shuncodeWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Shuncode", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Shuncode", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const shuncodeHooksDir = path.join(userDocumentsPath, "Shuncode", "Hooks")
	try {
		await fs.mkdir(shuncodeHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Shuncode", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return shuncodeHooksDir
}

/**
 * Returns the global skills directory path (~/.shuncode/skills).
 * Creates the directory if it doesn't exist.
 */
export async function ensureSkillsDirectoryExists(): Promise<string> {
	const shuncodeSkillsDir = path.join(getShuncodeHomePath(), "skills")
	try {
		await fs.mkdir(shuncodeSkillsDir, { recursive: true })
	} catch (_error) {
		// Fallback - return the path even if mkdir fails, we'll fail gracefully later
		return shuncodeSkillsDir
	}
	return shuncodeSkillsDir
}
...
export async function getSavedShuncodeMessages(taskId: string): Promise<ShuncodeMessage[]> {
...
export async function saveShuncodeMessages(taskId: string, uiMessages: ShuncodeMessage[]) {
	try {
...
export async function collectEnvironmentMetadata(): Promise<Omit<EnvironmentMetadataEntry, "ts">> {
	try {
		...
		return {
			...
			shuncode_version: ExtensionRegistryInfo.version,
		}
	} catch (error) {
...
export async function getTaskEvaluation(taskId: string): Promise<TaskEvaluation | undefined> {
	const filePath = path.join(await ensureTaskDirectoryExists(taskId), GlobalFileNames.taskEvaluation)
	try {
		if (await fileExistsAtPath(filePath)) {
			return JSON.parse(await fs.readFile(filePath, "utf8"))
		}
	} catch (error) {
		Logger.error("Failed to read task evaluation:", error)
	}
	return undefined
}

export async function saveTaskEvaluation(taskId: string, evaluation: TaskEvaluation): Promise<void> {
	try {
		const taskDir = await ensureTaskDirectoryExists(taskId)
		const filePath = path.join(taskDir, GlobalFileNames.taskEvaluation)
		await atomicWriteFile(filePath, JSON.stringify(evaluation, null, 2))
	} catch (error) {
		Logger.error("Failed to save task evaluation:", error)
	}
}
 ...
 * 2. Each workspace root's .shuncoderules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .shuncoderules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .shuncoderules/hooks folder in this workspace root.
