...
export class McpHub {
	...
	/**
	 * Flag to skip file watcher processing when we're updating Shuncode-specific settings
	 ...
	 */
	private isUpdatingShuncodeSettings: boolean = false
	...
	private async watchMcpSettingsFile(): Promise<void> {
		...
		this.settingsWatcher.on("change", async () => {
			...
			// Skip processing if we're updating Shuncode-specific settings (autoApprove, timeout)
			if (this.isUpdatingShuncodeSettings) {
				return
			}
	...
	private async connectToServer(
		...
		try {
			...
			const { transport, authProvider, createFallbackTransport } = await createTransport(name, config, this.mcpOAuthManager, {
			...
			try {
				...
				// If streamable HTTP failed and we have a fallback (legacy SSE), try it
				if (createFallbackTransport) {
					Logger.log(`Streamable HTTP failed for "${name}", falling back to legacy SSE transport`)
					try {
						...
						// Fallback succeeded - update the connection
						connection.client = fallbackClient
						connection.transport = fallbackTransport
					} catch (fallbackError) {
						if (fallbackError instanceof UnauthorizedError) {
							Logger.log(`Server "${name}" requires OAuth authentication (SSE fallback)`)
							const unauthConnection: McpConnection = {
								server: {
									name,
									config: JSON.stringify(config),
						...
						// Both transports failed - throw the original error
						throw error
					}
				} else {
	...
	async updateServerConnectionsRPC(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			if (!currentConnection) {
				...
				// Existing server with changed connection config (excludes Shuncode-specific settings)
				try {
					if (config.type === "stdio") {
						this.setupFileWatcher(name, config)
					}
				...
				// Only Shuncode-specific settings changed - update in-memory state without restart
				...
				// Also update Shuncode-specific settings in the stored config.
	...
	async updateServerConnections(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		// Track if any connection-level changes occurred (excludes Shuncode-specific settings)
		...
		// For Shuncode-specific settings changes, the RPC response already updated the webview,
	 ...
	 * Excludes Shuncode-specific settings since they don't affect the MCP server transport connection.
	 *
	 * ## Shuncode-specific settings (don't require restart):
	 ...
	 *
	 * ## Adding new Shuncode-specific settings:
	 ...
	 * 2. Add it to `isUpdatingShuncodeSettings` flag usage in the update function
	...
	private configsRequireRestart(oldConfig: McpServerConfig, newConfig: McpServerConfig): boolean {
		// Exclude Shuncode-specific settings from comparison (add new ones here)
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or Shuncode (and we want to detect save events, not every file change)
	...
	async toggleToolAutoApproveRPC(serverName: string, toolNames: string[], shouldAllow: boolean): Promise<McpServer[]> {
		...
		this.isUpdatingShuncodeSettings = true
		try {
			const settingsPath = await getMcpSettingsFilePathHelper(await this.getSettingsDirectoryPath())
			const content = await fs.readFile(settingsPath, "utf-8")
			const config = JSON.parse(content)
			...
			setTimeout(() => {
				this.isUpdatingShuncodeSettings = false
			}, 300)
		}
	}
