...
const FastContextDisplay: React.FC<FastContextDisplayProps> = ({ tool, isStreaming }) => {
	...
	const handleOpenFile = useCallback((filePath: string, startLine?: number) => {
		if (!filePath) return
		const value = startLine ? `${filePath}:${startLine}` : filePath
						...
						<div className="space-y-0.5">
							{foundFiles.map((file, i) => {
								...
								return (
									<button
										key={i}
										className="group flex items-center gap-2 text-xs py-0.5 px-1 rounded-sm w-full text-left cursor-pointer transition-colors duration-100 hover:bg-secondary/30 active:bg-secondary/50"
										onClick={() => handleOpenFile(file.filePath, file.startLine)}
										type="button">
