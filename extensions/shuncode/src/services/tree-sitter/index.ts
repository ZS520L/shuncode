...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = shuncodeIgnoreController ? shuncodeIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, shuncodeIgnoreController)
		if (definitions) {
...
/**
 * Parse a single file for code definitions (classes, functions, methods, etc.) with line numbers.
 * Used by list_code_definition_names when given a file path instead of a directory.
 * Returns formatted output like:
 *   L:  14 │ export class MyClass {
 *   L:  27 │ function helper() {
 */
export async function parseSourceCodeForDefinitionsSingleFile(
	filePath: string,
	shuncodeIgnoreController?: ShuncodeIgnoreController,
): Promise<string> {
	...
	if (!definitions) {
		return "No source code definitions found in this file."
	}
	...
	return `[File: ${path.basename(filePath)} | Total lines: ${totalLines}]\n${definitions}`
}
...
async function parseFile(
	...
	if (shuncodeIgnoreController && !shuncodeIgnoreController.validateAccess(filePath)) {
		return null
	}
	const fileContent = await fs.readFile(filePath, "utf8")
	...
	try {
		...
		captures.forEach((capture) => {
			...
			// Only add the first line of the definition — with line number for navigation
			...
			if (name.includes("name") && lines[startLine]) {
				const lineNum = String(startLine + 1).padStart(4)
				formattedOutput += `│${lineNum}│${lines[startLine]}\n`
			}
