...
import { RefreshedRules } from "@shared/proto/shuncode/file"
import { Logger } from "@/shared/services/Logger"
import { getCwd, getDesktopDir } from "@/utils/path"
import type { Controller } from "../index"

/**
 * Refreshes all rule toggles (Shuncode, External, and Workflows)
...
export async function refreshRules(controller: Controller, _request: EmptyRequest): Promise<RefreshedRules> {
	try {
		...
		const { globalToggles, localToggles } = await refreshShuncodeRulesToggles(controller, cwd)
