...
/**
 * Performs intialization for Shuncode that is common to all platforms.
 *
 * @param context
...
export async function initialize(context: vscode.ExtensionContext): Promise<WebviewProvider> {
	...
	try {
		...
		HostProvider.window.showMessage({
			...
			message: "Failed to initialize ShunCode's application state. Please restart the extension.",
		})
	}

	// Immediately sync announcement IDs and suppress banners on fresh install
	try {
		const sm = StateManager.get()
		const previousVersion = context.globalState.get<string>("shuncodeVersion")
		const isFreshInstall = !previousVersion

		// Sync announcement ID so shouldShowAnnouncement is false
		const lastShown = sm.getGlobalStateKey("lastShownAnnouncementId")
		const latestId = getLatestAnnouncementId()
		if (lastShown !== latestId) {
			sm.setGlobalState("lastShownAnnouncementId", latestId)
		}

		// Suppress remote banners on fresh install via a sentinel dismissal
		if (isFreshInstall) {
			const dismissed = sm.getGlobalStateKey("dismissedBanners") || []
			sm.setGlobalState("dismissedBanners", [
				...dismissed,
				{ bannerId: "__fresh_install_suppress_all__", dismissedAt: Date.now() },
			])
		}
	} catch {
		// StateManager may not be initialized; ignore
	}
	...
	// Migrate custom instructions to global Shuncode rules (one-time cleanup)
...
async function showVersionUpdateAnnouncement(context: vscode.ExtensionContext) {
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Shuncode version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const latestAnnouncementId = getLatestAnnouncementId()
			await context.globalState.update("lastShownAnnouncementId", latestAnnouncementId)
			try {
				StateManager.get().setGlobalState("lastShownAnnouncementId", latestAnnouncementId)
			} catch { }
			await context.globalState.update("shuncodeVersion", currentVersion)
		}
	} catch (error) {
 ...
 * If so, opens the Shuncode sidebar and clears the state.
 */
async function checkWorktreeAutoOpen(context: vscode.ExtensionContext): Promise<void> {
	try {
		...
		if (arePathsEqual(currentPath, worktreeAutoOpenPath)) {
			...
			// Open the Shuncode sidebar
			await HostProvider.workspace.openShuncodeSidebarPanel({})
		}
	} catch (error) {
...
/**
 * Performs cleanup when Shuncode is deactivated that is common to all platforms.
 */
