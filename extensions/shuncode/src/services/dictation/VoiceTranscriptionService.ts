...
const STATUS_ERROR_MAP = new Map<number, string>([
	[401, "Authentication failed. Please reauthenticate your Shuncode account"],
...
export class VoiceTranscriptionService {
	private readonly shuncodeAccountService: ShuncodeAccountService

	constructor() {
		this.shuncodeAccountService = ShuncodeAccountService.getInstance()
	}
	...
	async transcribeAudio(audioBase64: string, language?: string): Promise<{ text?: string; error?: string }> {
		try {
			Logger.info("Transcribing audio with ShunCode transcription service...")
			...
			const result = await this.shuncodeAccountService.transcribeAudio(audioBase64, language)
