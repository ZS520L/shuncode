...
import { ShuncodeIgnoreController } from "./ShuncodeIgnoreController"
import "should"

describe("ShuncodeIgnoreController", () => {
	let tempDir: string
	let controller: ShuncodeIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .shuncodeignore file
		await fs.writeFile(
			path.join(tempDir, ".shuncodeignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new ShuncodeIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .shuncodeignore file", async () => {
			const result = controller.validateAccess(".shuncodeignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .shuncodeignore", async () => {
			// Create a new .shuncodeignore with comments
			await fs.writeFile(
				path.join(tempDir, ".shuncodeignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .shuncodeignore gracefully", async () => {
			// Create a new controller in a directory without .shuncodeignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new ShuncodeIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .shuncodeignore", async () => {
			await fs.writeFile(path.join(tempDir, ".shuncodeignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .shuncodeignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".shuncodeignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .shuncodeignore
			controller = new ShuncodeIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .shuncodeignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .shuncodeignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".shuncodeignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".shuncodeignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
