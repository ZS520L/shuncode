...
import { ShuncodeAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/**
	 * The extension or shuncode-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _shuncode-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of shuncode distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
...
/**
 * TelemetryService handles telemetry event tracking for the Shuncode extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "shuncode.turns.total",
			TURNS_PER_TASK: "shuncode.turns.per_task",
			TOKENS_INPUT_TOTAL: "shuncode.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "shuncode.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "shuncode.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "shuncode.tokens.output.per_response",
			COST_TOTAL: "shuncode.cost.total",
			COST_PER_EVENT: "shuncode.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "shuncode.cache.write.tokens.total",
			WRITE_PER_EVENT: "shuncode.cache.write.tokens.per_event",
			READ_TOTAL: "shuncode.cache.read.tokens.total",
			READ_PER_EVENT: "shuncode.cache.read.tokens.per_event",
			HITS_TOTAL: "shuncode.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "shuncode.tool.calls.total",
			CALLS_PER_TASK: "shuncode.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "shuncode.errors.total",
			PER_TASK: "shuncode.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "shuncode.api.ttft.seconds",
			DURATION_SECONDS: "shuncode.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "shuncode.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "shuncode.hooks.executions.total",
			DURATION_SECONDS: "shuncode.hooks.duration.seconds",
			FAILURES_TOTAL: "shuncode.hooks.failures.total",
			CANCELLATIONS_TOTAL: "shuncode.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "shuncode.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "shuncode.hooks.cache.accesses.total",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual Shuncode rules are toggled on/off
			...
			// Tracks when Shuncode web tools setting is toggled on/off
			SHUNCODE_WEB_TOOLS_TOGGLED: "task.shuncode_web_tools_toggled",
	...
	public static async create(): Promise<TelemetryService> {
		...
		const metadata: TelemetryMetadata = {
			...
			shuncode_type: hostVersion.shuncodeType || "unknown",
			os_type: os.platform(),
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		// --- SHUNCODE_FORK_BEGIN: suppress telemetry warning ---
	 ...
	 */
	public identifyAccount(userInfo: ShuncodeAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when shuncode calls the task completion_result tool signifying that shuncode is done with the task
	...
	/**
	 * Records when individual Shuncode rules are toggled on/off
	 ...
	 */
	public captureShuncodeRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
	...
	/**
	 * Records when Shuncode web tools are enabled/disabled by the user
	 * @param ulid Unique identifier for the task
	 * @param enabled Whether Shuncode web tools are enabled (true) or disabled (false)
	 */
	public captureShuncodeWebToolsToggle(ulid: string, enabled: boolean) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.SHUNCODE_WEB_TOOLS_TOGGLED,
			properties: {
				ulid,
				enabled,
			},
	...
	public captureWorkspaceInitialized(
		...
		this.recordGauge("shuncode.workspace.active_roots", rootCount, {
			is_multi_root: isMultiRoot,
		})
		// Retire the previous series to avoid leaking gauge entries when the flag flips.
		this.recordGauge("shuncode.workspace.active_roots", null, {
			is_multi_root: !isMultiRoot,
		})
	}
