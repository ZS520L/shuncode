...
export class FastContextToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			let result = await agent.search(query, (progress) => {
			...
			// --- SHUNCODE: Auto-retry with wider scope if confidence is low ---
			// When the sub-agent reports low confidence and a scope was provided,
			// automatically retry without scope restriction to search the full workspace.
			if (result.success && result.confidence === "low" && scope) {
				const retryMsg = JSON.stringify({
					tool: "fastContext",
					query,
					status: "searching",
					content: "Low confidence in scoped results — retrying with full workspace...",
					turns,
				})
				await config.callbacks.removeLastPartialMessageIfExistsWithType("ask", "tool")
				await config.callbacks.say("tool", retryMsg, undefined, undefined, true)

				result = await agent.search(query, (progress) => {
					const turnIdx = (progress.currentTurn || 1) - 1
					if (!turns[turnIdx]) {
						turns[turnIdx] = { turnNumber: progress.currentTurn || 1, operations: [] }
					}
					...
					const msg = JSON.stringify({
						...
						maxTurns: progress.maxTurns,
						turns,
						content: progress.message || `Retry turn ${progress.currentTurn}/${progress.maxTurns}`,
					})
					config.callbacks.say("tool", msg, undefined, undefined, true)
				}, { scope: undefined, directoryHints })
			}
