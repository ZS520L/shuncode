...
describe("disk - hooks functionality", () => {
	...
	describe("getWorkspaceHooksDirs", () => {
		...
		it("should return hooks directory when it exists", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksDir = path.join(workspaceRoot, ".shuncoderules", "hooks")
		...
		it("should not return hooks directory if it's a file instead of directory", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksPath = path.join(workspaceRoot, ".shuncoderules", "hooks")
		...
		it("should return multiple hooks directories for multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".shuncoderules", "hooks")
			const hooksDir2 = path.join(workspaceRoot2, ".shuncoderules", "hooks")
		...
		it("should return only existing hooks directories in multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".shuncoderules", "hooks")
			const hooksDir3 = path.join(workspaceRoot3, ".shuncoderules", "hooks")
			...
			result.should.not.containEql(path.join(workspaceRoot2, ".shuncoderules", "hooks"))
		})
		...
		it("should use correct path joining for hooks directory", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			const expectedHooksDir = path.join(workspaceRoot, ".shuncoderules", "hooks")
			...
			result[0].should.match(/\.shuncoderules[\\/]hooks$/)
		})
...
describe("disk - atomic writes", () => {
	...
	before(async () => {
		...
		testGlobalStorageDir = path.join(os.tmpdir(), `shuncode-test-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`)
