...
const ContextMenu: React.FC<ContextMenuProps> = ({
	...
	const SIMPLE_OPTION_LABELS: Partial<Record<ContextMenuOptionType, string>> = {
		[ContextMenuOptionType.Problems]: t("chat.context.problems"),
		[ContextMenuOptionType.Terminal]: t("chat.context.terminal"),
		[ContextMenuOptionType.URL]: t("chat.context.pasteUrlToFetch"),
		[ContextMenuOptionType.NoResults]: t("chat.context.noResults"),
	}
	...
	const getOptionLabel = useCallback((option: ContextMenuQueryItem): string => {
		...
		switch (option.type) {
			case ContextMenuOptionType.Git:
				...
				return t("chat.context.gitCommits")
			case ContextMenuOptionType.File:
			case ContextMenuOptionType.Folder:
				if (option.value) {
					return option.label || option.value
				}
				return `${t("chat.context.add")} ${option.type === ContextMenuOptionType.File ? t("chat.context.file") : t("chat.context.folder")}`
			default:
				return option.label || option.value || ""
		}
	}, [])
	...
	const renderOptionContent = (option: ContextMenuQueryItem) => {
		...
		switch (option.type) {
			case ContextMenuOptionType.Git:
				...
				return <span>{t("chat.context.gitCommits")}</span>
			case ContextMenuOptionType.File:
			case ContextMenuOptionType.Folder:
				if (option.value) {
				...
				return (
					<span>
						{t("chat.context.add")}{" "}
						{option.type === ContextMenuOptionType.File ? t("chat.context.file") : t("chat.context.folder")}
					</span>
				)
			default:
				return null
	...
	return (
		<div
			...
			<div
				...
				aria-label={t("chat.context.mentions")}
				ref={menuRef}
				role="listbox"
				style={{
				...
				{showDelayedLoading && searchQuery && (
					<div
						...
						<span>{t("chat.context.searching")}</span>
					</div>
				)}
