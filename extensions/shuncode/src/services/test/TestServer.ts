import { getSavedApiConversationHistory, getSavedShuncodeMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Try to show the Shuncode sidebar
	Logger.log("[createTestServer] Opening Shuncode in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Shuncode instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "shuncode" as ApiProvider,
							shuncodeAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("shuncodeAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use shuncode provider
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedShuncodeMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Shuncode messages: ${error}`)
						}
