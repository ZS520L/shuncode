...
import { WorktreeDefaults } from "@shared/proto/shuncode/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/shuncode-${suffix}`

	// Generate suggested path in Documents/Shuncode/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Shuncode", "Worktrees", `${projectName}-${suffix}`)
