...
describe("Remote Config Schema", () => {
	...
	describe("ShuncodeSettingsSchema", () => {
		it("should accept valid Shuncode provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = ShuncodeSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = ShuncodeSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => ShuncodeSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				ShuncodeSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			...
			// Verify Shuncode settings
			expect(result.providerSettings?.Shuncode?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.Shuncode?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.Shuncode?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
