...
export interface FastContextResult {
	...
	tokensUsed?: number
	/**
	 * Sub-agent's self-assessed confidence in the results.
	 * 'high' = strong match found, directly answers the query.
	 * 'medium' = partial/related match, may not fully answer.
	 * 'low' = no strong match in the searched scope; caller should consider retrying with wider scope.
	 */
...
export interface FastContextProgress {
	...
	operations?: Array<{
		type: "grep" | "read_file" | "find_files" | "ls"
		args: string
		status: "running" | "done"
		duration?: number
	}>
