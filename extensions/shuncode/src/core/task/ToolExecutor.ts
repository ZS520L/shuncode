...
import { ReadTerminalToolHandler } from "./tools/handlers/ReadTerminalToolHandler"
...
import { EvaluateTaskHandler } from "./tools/handlers/EvaluateTaskHandler"
import { ReadDiagnosticsToolHandler } from "./tools/handlers/ReadDiagnosticsToolHandler"
import { HashlineEditToolHandler } from "./tools/handlers/HashlineEditToolHandler"
import { ReadFilesToolHandler } from "./tools/handlers/ReadFilesToolHandler"
...
export class ToolExecutor {
	...
	private registerToolHandlers(): void {
		...
		this.coordinator.register(new ReadTerminalToolHandler())
		...
		this.coordinator.register(new EvaluateTaskHandler())
		this.coordinator.register(new HashlineEditToolHandler(validator))
		this.coordinator.register(new ReadFilesToolHandler(validator))
	}
	...
	/**
	 * Execute multiple tool blocks, running concurrency-safe ones in parallel.
	 * Non-safe tools are executed sequentially after any parallel batch completes.
	 * This method is called when parallel tool calling is enabled and multiple
	 * tool_use blocks arrive in the same assistant turn.
	 */
	public async executeToolBatch(blocks: ToolUse[]): Promise<void> {
		if (blocks.length === 0) return
		if (blocks.length === 1) {
			await this.execute(blocks[0])
			return
		}

		// Partition into concurrent-safe and non-safe groups (preserving order)
		const safeBatch: ToolUse[] = []
		const rest: ToolUse[] = []
		for (const block of blocks) {
			if (this.coordinator.isConcurrencySafeForTool(block.name)) {
		...
		if (safeBatch.length > 1) {
			...
			const results = await this.coordinator.executeParallel(config, safeBatch)
			for (const { block, result } of results) {
				this.pushToolResult(result, block)
				this.trackFailureLoop(block, result)
				this.taskState.lastToolName = block.name
				this.trackToolCallForSessionBudget(block)
			}
		} else if (safeBatch.length === 1) {
			await this.execute(safeBatch[0])
		}
	...
	private async execute(block: ToolUse): Promise<boolean> {
		...
		try {
			...
			if (this.taskState.didRejectTool) {
				this.taskState.evaluationTracker.recordToolRejected(block.name)
			...
			if (
				...
				this.taskState.evaluationTracker.recordModeViolation(block.name, currentMode)
			...
			if (
				...
				this.taskState.evaluationTracker.recordModeViolation(block.name, "lightweight")
				this.taskState.consecutiveMistakeCount++
	...
	/**
	 * Extracts target file path from tool params for failure loop tracking.
	 */
	private getTargetFileFromParams(params: Record<string, unknown>): string | undefined {
		return (params.absolutePath ?? params.path ?? params.target_notebook ?? params.relPath) as string | undefined
	}

	/**
	 * Tracks repeated failures on the same tool+file combination.
	 * When the same tool fails on the same target 2+ times consecutively,
	 * injects an escalation warning to prompt the model to try a different approach.
	 *
	 * This implements the "failed twice → step back and rethink" discipline
	 * at the runtime level, not just in the system prompt.
	 */
	private trackFailureLoop(block: ToolUse, toolResult: any): void {
		const resultText = typeof toolResult === "string"
			? toolResult
			: Array.isArray(toolResult)
				? toolResult.map((r: any) => (typeof r === "string" ? r : r?.text ?? "")).join("\n")
				: ""

		// Detect if this tool call resulted in a failure
		const isFailure =
			/\b(error|failed|not found|no match|does not exist|ENOENT)\b/i.test(resultText) &&
			resultText.length < 2000 // Avoid false positives on large successful outputs

		const targetFile = this.getTargetFileFromParams(block.params)
		const toolName = block.name ?? ""

		if (!isFailure) {
			// Success — clear the failure context if it was for this tool+file
			if (
				this.taskState.lastFailedToolContext &&
				this.taskState.lastFailedToolContext.toolName === toolName &&
				this.taskState.lastFailedToolContext.targetFile === targetFile
			) {
				this.taskState.lastFailedToolContext = null
			}
			return
		}

		// This is a failure — check if it's a repeat
		const ctx = this.taskState.lastFailedToolContext
		if (ctx && ctx.toolName === toolName && ctx.targetFile === targetFile) {
			ctx.count++
		} else {
			this.taskState.lastFailedToolContext = { toolName, targetFile, count: 1 }
		}

		const failCtx = this.taskState.lastFailedToolContext!
		if (failCtx.count >= 2) {
			this.taskState.repeatedFailureLoopCount++
			this.taskState.evaluationTracker.recordRepeatedFailureLoop()
			const targetDesc = targetFile ? ` on "${targetFile}"` : ""
			this.taskState.userMessageContent.push({
				type: "text",
				text: `[FAILURE LOOP DETECTED] Tool "${toolName}" has failed ${failCtx.count} times consecutively${targetDesc}. ` +
					`Stop repeating the same approach. Diagnose the root cause: re-read the file, check your assumptions, ` +
					`and try a fundamentally different strategy. If using replace_in_file, verify the SEARCH content matches exactly.`,
			})
			// Reset count so we don't spam on every subsequent call
			failCtx.count = 0
		}
	}
	...
	private trackToolCallForSessionBudget(block: ToolUse): void {
		...
		if (this.taskState.consecutiveReadOnlyToolCalls >= limits.maxConsecutiveReadOnlyTools) {
			this.taskState.evaluationTracker.recordSessionGuardrail("exploration_loop")
			this.taskState.userMessageContent.push({
				type: "text",
		...
		if (this.taskState.turnToolCallCount >= limits.maxToolCallsPerTurn) {
			...
			this.taskState.evaluationTracker.recordSessionGuardrail("budget_exhausted")
			this.taskState.userMessageContent.push({
				type: "text",
	...
	private async handleCompleteBlock(block: ToolUse, config: any): Promise<void> {
		...
		try {
			...
			this.taskState.evaluationTracker.recordToolUse({
				toolName: block.name,
				status: "success",
			...
			// Track failure loops — inject escalation warning if same tool+file fails repeatedly
			this.trackFailureLoop(block, toolResult)
