...
import { buildBasicShuncodeHeaders } from "../EnvUtils"

export class ShuncodeAccountService {
	private static instance: ShuncodeAccountService
	...
	/**
	 * Returns the singleton instance of ShuncodeAccountService
	 * @returns Singleton instance of ShuncodeAccountService
	 */
	public static getInstance(): ShuncodeAccountService {
		if (!ShuncodeAccountService.instance) {
			ShuncodeAccountService.instance = new ShuncodeAccountService()
		}
		return ShuncodeAccountService.instance
	}

	/**
	 * Returns the base URL for the Shuncode API
	 * @returns The base URL as a string
	 */
	get baseUrl(): string {
		return ShuncodeEnv.config().apiBaseUrl
	}

	/**
	 * Helper function to make authenticated requests to the Shuncode API
	...
	private async authenticatedRequest<T>(endpoint: string, config: AxiosRequestConfig = {}): Promise<T> {
		...
		if (!shuncodeAccountAuthToken) {
			throw new Error("No Shuncode account auth token found")
		}
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(SHUNCODE_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(SHUNCODE_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
	...
	/**
	 * Transcribes audio using the Shuncode transcription service
