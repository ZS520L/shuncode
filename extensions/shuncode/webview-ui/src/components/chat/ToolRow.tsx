...
export function ToolRow({ tool, message, backgroundEditEnabled, isExpanded, onToggleExpand }: ToolRowProps) {
	...
	switch (tool.tool) {
		...
		case "readFile": {
			...
			const canOpen = !isImage && !isStreaming
			return (
				<div
					className="flex items-center gap-2 py-1 px-1 text-[13px] text-description cursor-pointer hover:text-foreground transition-colors"
					onClick={() => {
						if (canOpen) {
					...
					{isImage ? <ImageUpIcon className="size-3.5 shrink-0 opacity-70" /> : <FileCode2Icon className="size-3.5 shrink-0 opacity-70" />}
					...
					<span className="ph-no-capture min-w-0 truncate text-link hover:underline">
						{(tool.path ?? "").split(/[/\\]/).pop() || cleanPathPrefix(tool.path ?? "")}
					</span>
