...
const HERMES_TOOL_USE_TEMPLATE = `Begin every task by exploring the codebase, using fast_context first for semantic queries, list_files/read_file for scope and deep inspection, and search_files only for exact patterns in narrow paths. Do not implement until exploration yields enough context to state objectives, approach, affected files, and risks. Briefly summarize the plan, then proceed with implementation.
...
**execute_command** â€?Run terminal commands in {{CWD}} or other directories.
...
**write_to_file** â€?Create/overwrite file. You should only use this when editing a new file.
...
**replace_in_file** â€?Targeted edits to perform on existing files. You should use replace_in_file when editing a file that already exists.
Params: path, diff
...
**search_files** â€?Regex search to perform.
...
Key: Rely on returned tool results instead of using list_files to â€œconfirmâ€?writes.

**attempt_completion** â€?Final result (no questions). Use this tool only when all goals have been completed.
...
**new_task** â€?Create a new task with context.
...
**plan_mode_respond** â€?PLAN-only reply.
...
- Include the full checklist of meaningful milestonesâ€”not low-level technical steps.
