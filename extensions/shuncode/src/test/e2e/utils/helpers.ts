...
import { ShuncodeApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to Shuncode" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to Shuncode" })).not.toBeVisible()

		await webview.getByRole("button", { name: "Close" }).click({ delay: 50 })
	}

	public static async openShuncodeSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /Shuncode/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Shuncode extension.
 *
 * Extended Playwright test configuration for Shuncode E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Shuncode VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared ShuncodeApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Shuncode sidebar opened
 * - `sidebar`: Playwright Frame object representing the Shuncode extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a ShuncodeApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Shuncode sidebar open
 * - **sidebar**: Provides access to the Shuncode extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Shuncode extension in development mode
...
export const e2e = test
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openShuncodeSidebar(page)
