...
const AutoApproveBar = ({ style }: AutoApproveBarProps) => {
	...
	const getEnabledActionsText = () => {
		...
		if (actionsToShow.length === 0) {
			return <span className={baseClasses}>{t("chat.none")}</span>
		}
		...
		return (
			<span className={baseClasses}>
				{actionsToShow.map((action, index) => (
					<span key={action?.id}>
						{action?.shortNameKey ? t(action.shortNameKey) : action?.id}
	...
	return (
		<div
			...
			<div
				aria-label={isModalVisible ? t("chat.closeAutoApproveSettings") : t("chat.openAutoApproveSettings")}
				...
				<div className="flex flex-nowrap items-center gap-1 min-w-0 flex-1">
					<span className="whitespace-nowrap">{t("chat.autoApprove")}:</span>
