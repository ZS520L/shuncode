...
export class DeepSeekHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: ShuncodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		...
		const effort = this.options.reasoningEffort || undefined
		...
		if (isV4 && effort) {
			requestParams["thinking"] = { type: "enabled" as const }
			requestParams["reasoning_effort"] = effort
		} else if (isV4) {
			// V4 without specified effort: non-thinking mode
			requestParams["thinking"] = { type: "disabled" as const }
		}

		const stream = await (client as any).chat.completions.create(requestParams)
