import type { ShuncodeAsk, ShuncodeSay } from "@shared/ExtensionMessage"
...
export interface StronglyTypedUIHelpers {
	// Core UI methods
	say: (type: ShuncodeSay, text?: string, images?: string[], files?: string[], partial?: boolean) => Promise<number | undefined>
	...
	removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: ShuncodeAsk | ShuncodeSay) => Promise<void>

	// Approval methods
	shouldAutoApproveTool: (toolName: ShuncodeDefaultTool) => boolean | [boolean, boolean]
	shouldAutoApproveToolWithPath: (toolName: ShuncodeDefaultTool, path?: string) => Promise<boolean>
	askApproval: (messageType: ShuncodeAsk, message: string) => Promise<boolean>

	// Telemetry and notifications
	captureTelemetry: (toolName: ShuncodeDefaultTool, autoApproved: boolean, approved: boolean, isNativeToolCall?: boolean) => void
...
export function createUIHelpers(config: TaskConfig): StronglyTypedUIHelpers {
	return {
		...
		shouldAutoApproveTool: (toolName: ShuncodeDefaultTool) => config.autoApprover.shouldAutoApproveTool(toolName),
		shouldAutoApproveToolWithPath: config.callbacks.shouldAutoApproveToolWithPath,
		askApproval: async (messageType: ShuncodeAsk, message: string): Promise<boolean> => {
			const { response } = await config.callbacks.ask(messageType, message, false)
			return response === "yesButtonClicked"
		},
		captureTelemetry: (toolName: ShuncodeDefaultTool, autoApproved: boolean, approved: boolean, isNativeToolCall?: boolean) => {
