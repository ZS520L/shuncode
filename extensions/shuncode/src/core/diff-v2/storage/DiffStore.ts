...
export class DiffStore implements vscode.Disposable {
  ...
  /**
   * Override the kind of an existing FileChange.
   * Used when a file is first registered as 'modified' (e.g. via addLines on a
   * freshly-created empty file) but is actually a brand-new file creation. The
   * correct kind matters for rollback, which deletes 'created' files entirely
   * instead of restoring them to an empty snapshot.
   */
  updateFileChangeKind(id: string, kind: FileChangeKind): void {
    const all = this.getFCs();
    const idx = all.findIndex((fc) => fc.id === id);
    if (idx !== -1 && all[idx].kind !== kind) {
      all[idx].kind = kind;
      this._dirtyKeys.add(DiffStore.FC_KEY);
      this.schedulePersist();
    }
