...
export async function getUserInstructions(variant: PromptVariant, context: SystemPromptContext): Promise<string | undefined> {
	const customInstructions = buildUserInstructions(
		context.localShuncodeRulesFileInstructions,
		...
		context.shuncodeIgnoreInstructions,
...
function buildUserInstructions(
	localShuncodeRulesFileInstructions?: string,
	localCursorRulesFileInstructions?: string,
	localCursorRulesDirInstructions?: string,
	localWindsurfRulesFileInstructions?: string,
	localAgentsRulesFileInstructions?: string,
	shuncodeIgnoreInstructions?: string,
	...
	// Note: globalShuncodeRulesFileInstructions intentionally omitted —
	// already covered by {{pinnedMemory}} in the template to avoid duplication.
	if (localShuncodeRulesFileInstructions) {
		customInstructions.push(localShuncodeRulesFileInstructions)
	}
	...
	if (shuncodeIgnoreInstructions) {
		customInstructions.push(shuncodeIgnoreInstructions)
	}
