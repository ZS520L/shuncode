...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "shuncode-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .shuncoderules/workflows directory specifically", async () => {
		// Create a test directory structure
		const shuncoderulesDirTest = path.join(tmpDir, "shuncoderules-test")
		const shuncoderulesDirPath = path.join(shuncoderulesDirTest, ".shuncoderules")

		// Create .shuncoderules directory and root files
		await fs.mkdir(shuncoderulesDirPath, { recursive: true })
		await fs.writeFile(path.join(shuncoderulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(shuncoderulesDirPath, "settings.js"), "// settings")

		// Create .shuncoderules/other directory and files
		const otherDirPath = path.join(shuncoderulesDirPath, "other")
		...
		// Create .shuncoderules/workflows directory and files
		const workflowsDirPath = path.join(shuncoderulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(shuncoderulesDirPath)
		...
		const filteredFiles = await readDirectory(shuncoderulesDirPath, [[".shuncoderules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(shuncoderulesDirPath, "config.json"),
			path.resolve(shuncoderulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(shuncoderulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(shuncoderulesDirPath, "config.json"), path.resolve(shuncoderulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .shuncoderules/hooks directory specifically", async () => {
		// Create a test directory structure
		const shuncoderulesDirTest = path.join(tmpDir, "shuncoderules-hooks-test")
		const shuncoderulesDirPath = path.join(shuncoderulesDirTest, ".shuncoderules")
		...
		// Create .shuncoderules/hooks directory and files
		const hooksDirPath = path.join(shuncoderulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(shuncoderulesDirPath, [[".shuncoderules", "hooks"]])
