/**
 * ShuncodeTempManager - Manages temporary files for Shuncode with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "shuncode" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Shuncode's temporary files.
 */
class ShuncodeTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "shuncode" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/shuncode
		// Windows: C:\Users\{user}\AppData\Local\Temp\shuncode
		// Linux: /tmp/shuncode
		const baseTempDir = os.tmpdir()
		const shuncodeTempDir = path.join(baseTempDir, "shuncode")

		try {
			fs.mkdirSync(shuncodeTempDir, { recursive: true })
			this.tempDir = shuncodeTempDir
		} catch {
	...
	/**
	 * Clean up old Shuncode temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Shuncode temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Shuncode temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Shuncode temp cleanup", error)
		}
...
export const ShuncodeTempManager = new ShuncodeTempManagerImpl()
