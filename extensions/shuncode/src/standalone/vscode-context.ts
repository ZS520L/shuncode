...
log("Running standalone shuncode", ExtensionRegistryInfo.version)
log(`SHUNCODE_ENVIRONMENT: ${process.env.SHUNCODE_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(shuncodeDir?: string) {
	const SHUNCODE_DIR = shuncodeDir || process.env.SHUNCODE_DIR || `${os.homedir()}/.shuncode`
	const DATA_DIR = path.join(SHUNCODE_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in shuncode.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in shuncode.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in shuncode.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in shuncode.
