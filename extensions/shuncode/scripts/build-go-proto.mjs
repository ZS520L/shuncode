...
export async function goProtoc(outDir, protoFiles) {
	...
	const goProtocCommand = [
		...
		`--go_out="${outDir}"`,
		`--go_opt=module=github.com/shuncode/grpc-go`,
		`--go-grpc_out="${outDir}"`,
		`--go-grpc_opt=module=github.com/shuncode/grpc-go`,
		...protoFiles,
	].join(" ")
...
async function generateGoMod() {
	...
	const goModContent = `module github.com/shuncode/grpc-go
...
import (
	...
	"google.golang.org/grpc"
	"github.com/shuncode/grpc-go/client/services"
)

// ShuncodeClient provides a unified interface to all Shuncode services
type ShuncodeClient struct {
...
// NewShuncodeClient creates a new unified Shuncode client
func NewShuncodeClient(address string) (*ShuncodeClient, error) {
	config := &ConnectionConfig{
		Address: address,
	}
	...
	return &ShuncodeClient{
		connManager: connManager,
	}, nil
}

// NewShuncodeClientWithConfig creates a new Shuncode client with custom configuration
func NewShuncodeClientWithConfig(config *ConnectionConfig) (*ShuncodeClient, error) {
	connManager := NewConnectionManager(config)
...
// Connect establishes connection to Shuncode Core and initializes service clients
func (c *ShuncodeClient) Connect(ctx context.Context) error {
	c.mutex.Lock()
...
// Disconnect closes the connection to Shuncode Core
func (c *ShuncodeClient) Disconnect() error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
// IsConnected returns true if the client is connected to Shuncode Core
func (c *ShuncodeClient) IsConnected() bool {
	c.mutex.RLock()
...
func (c *ShuncodeClient) Reconnect(ctx context.Context) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *ShuncodeClient) GetConnection() *grpc.ClientConn {
	return c.connManager.GetConnection()
}
`
	const clientPath = path.join(GO_CLIENT_DIR, "shuncode_client.go")
...
func (sc *${capitalizedServiceName}Client) ${capitalizedMethodName}(ctx context.Context, req ${requestType}) (${responseType}, error) {
	...
	return resp, nil
		...
		const protoImportPath =
			serviceDef.protoPackage === "host" ? '"github.com/shuncode/grpc-go/host"' : '"github.com/shuncode/grpc-go/shuncode"'

		// Check if we need to import shuncode package for common types
		const needsShuncodeImport = serviceDef.methods.some((method) => {
			const requestTypeName = method.requestType.split(".").pop()
			const responseTypeName = method.responseType.split(".").pop()
		...
		// Always import shuncode package if we need common types, regardless of service package
		const shuncodeImport = needsShuncodeImport ? '	shuncode "github.com/shuncode/grpc-go/shuncode"\n' : ""
...
${shuncodeImport}	proto ${protoImportPath}
	"google.golang.org/grpc"
)
