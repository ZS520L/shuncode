	...
	return (
		<div className="flex flex-col gap-1">
			<VSCodeRadioGroup
				...
				<VSCodeRadio value="apikey">{t("provider.apiKey")}</VSCodeRadio>
				<VSCodeRadio value="profile">{t("provider.awsProfile")}</VSCodeRadio>
				<VSCodeRadio value="credentials">{t("provider.awsCredentials")}</VSCodeRadio>
			</VSCodeRadioGroup>
			...
			apiConfiguration?.awsAuthentication === "profile" ? (
				<DebouncedTextField
					...
					placeholder={t("provider.enterProfileName")}>
					<span className="font-medium">{t("provider.awsProfileName")}</span>
				</DebouncedTextField>
				...
				<DebouncedTextField
					...
					placeholder={t("provider.enterBedrockApiKey")}
					type="password">
					<span className="font-medium">{t("provider.awsBedrockApiKey")}</span>
				</DebouncedTextField>
			) : (
				<>
					<DebouncedTextField
						...
						placeholder={t("provider.enterAccessKey")}
						type="password">
						<span className="font-medium">{t("provider.awsAccessKey")}</span>
					</DebouncedTextField>
					<DebouncedTextField
						className="w-full"
						initialValue={apiConfiguration?.awsSecretKey || ""}
						onChange={(value) => handleFieldChange("awsSecretKey", value)}
						placeholder={t("provider.enterSecretKey")}
						type="password">
						<span className="font-medium">{t("provider.awsSecretKey")}</span>
					</DebouncedTextField>
					<DebouncedTextField
						className="w-full"
						initialValue={apiConfiguration?.awsSessionToken || ""}
						onChange={(value) => handleFieldChange("awsSessionToken", value)}
						placeholder={t("provider.enterSessionToken")}
						type="password">
						<span className="font-medium">{t("provider.awsSessionToken")}</span>
					</DebouncedTextField>
				</>
			)}
				...
				<TooltipTrigger>
					<DropdownContainer className="dropdown-container mb-2.5" zIndex={DROPDOWN_Z_INDEX - 1}>
						...
						<VSCodeDropdown
							...
							<VSCodeOption value="">{t("provider.selectRegion")}</VSCodeOption>
					...
					<TooltipTrigger>
						<div className="flex items-center gap-2">
							<VSCodeCheckbox
								...
								{t("provider.useCustomVpcEndpoint")}
							</VSCodeCheckbox>
						...
						{awsEndpointSelected && (
							<DebouncedTextField
								...
								placeholder={t("provider.enterVpcEndpointUrl")}
								type="text"
							/>
						)}
					</TooltipTrigger>
					...
					<TooltipTrigger>
						<div className="flex items-center gap-2">
							<VSCodeCheckbox
								...
								{t("provider.useCrossRegionInference")}
							</VSCodeCheckbox>
						...
						<TooltipTrigger>
							<div className="flex items-center gap-2">
								<VSCodeCheckbox
									...
									{t("provider.useGlobalInferenceProfile")}
								</VSCodeCheckbox>
			...
			<p className="mt-1 text-sm text-description">
				{apiConfiguration?.awsUseProfile
					? t("provider.awsProfileCredentialsDescription")
					: t("provider.awsCredentialsDescription")}
			</p>
				...
				<>
					...
					<DropdownContainer className="dropdown-container" zIndex={DROPDOWN_Z_INDEX - 2}>
						<VSCodeDropdown
							...
							<VSCodeOption value="custom">{t("provider.custom")}</VSCodeOption>
						</VSCodeDropdown>
					</DropdownContainer>

					{modeFields.awsBedrockCustomSelected && (
						<div>
							<p className="mt-1 text-sm text-description">{t("provider.bedrockCustomHint")}</p>
							<DebouncedTextField
								...
								placeholder={t("provider.enterCustomModelId")}>
								<span className="font-medium">{t("provider.modelId")}</span>
							</DebouncedTextField>
							<label htmlFor="bedrock-base-model-dropdown">
								<span className="font-medium">{t("provider.baseInferenceModel")}</span>
							</label>
