...
/**
 * Discover all skills from the global user skills directory (~/.shuncode/skills).
 */
export async function discoverSkills(_cwd: string): Promise<SkillMetadata[]> {
	const globalSkillsDir = await ensureSkillsDirectoryExists()
	return scanSkillsDirectory(globalSkillsDir, "global")
}
