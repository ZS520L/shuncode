	...
	return (
		<div className="flex flex-col gap-2">
			<BaseUrlField
				...
				placeholder={t("provider.defaultLmStudioBaseUrl")}
			/>

			<div className="font-semibold">{t("provider.model")}</div>
			{lmStudioModels.length > 0 ? (
				...
				<DebouncedTextField
					...
					placeholder={t("provider.exampleLmStudioModel")}
					style={{ width: "100%" }}
				/>
			)}

			<div className="font-semibold">{t("provider.modelContextWindow")}</div>
			<VSCodeTextField
				className="w-full pointer-events-none"
				disabled={true}
				title={t("provider.notEditableReturnedByEndpoint")}
			...
			<div className="text-xs text-description">
				{t("provider.lmStudioDescriptionPrefix")}
				<VSCodeLink href="https://lmstudio.ai/docs" style={{ display: "inline", fontSize: "inherit" }}>
					{t("provider.quickstartGuide")}
				</VSCodeLink>
				{t("provider.lmStudioNeedStartPrefix")}{" "}
				<VSCodeLink className="inline" href="https://lmstudio.ai/docs/basics/server">
					{t("provider.localServer")}
				</VSCodeLink>{" "}
				{t("provider.lmStudioNeedStartSuffix")} <div className="text-error">{t("provider.noteComplexPrompts")}</div>
			</div>
		</div>
	)
}
