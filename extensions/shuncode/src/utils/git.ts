...
/**
 * Get compact git status for system prompt context
 * Returns branch name and change summary
 */
export interface GitStatusCompact {
	branch?: string
	hasChanges: boolean
	summary?: string // e.g. "2 modified, 1 untracked"
}

export async function getGitStatusCompact(cwd: string): Promise<GitStatusCompact | null> {
	try {
		const isInstalled = await checkGitInstalled()
		if (!isInstalled) {
			return null
		...
		Logger.error("Error getting compact git status:", error)
		return null
	}
}
