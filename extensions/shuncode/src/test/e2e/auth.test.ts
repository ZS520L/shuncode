...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	await expect(sidebar.getByRole("button", { name: "Login to Shuncode" })).toBeVisible()
	...
	// Test Shuncode provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find Shuncode option
	await expect(sidebar.getByTestId("provider-option-shuncode")).toBeVisible()
	await sidebar.getByTestId("provider-option-shuncode").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with Shuncode" })).toBeVisible()
	...
	await expect(sidebar.getByRole("button", { name: "Login to Shuncode" })).not.toBeVisible()
	...
	// shuncode logo container
	const shuncodeLogo = sidebar.locator(".size-20")
	await expect(shuncodeLogo).toBeVisible()
