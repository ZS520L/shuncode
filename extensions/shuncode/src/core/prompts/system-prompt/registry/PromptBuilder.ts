...
import type { ShuncodeDefaultTool } from "@/shared/tools"
import { ShuncodeToolSet } from "../registry/ShuncodeToolSet"
import { type ShuncodeToolSpec, resolveInstruction } from "../spec"
...
export class PromptBuilder {
	...
	async build(): Promise<string> {
		...
		const template = this.context.customSystemPromptTemplate || this.variant.baseTemplate
		const prompt = this.templateEngine.resolve(template, this.context, placeholderValues)
	...
	private preparePlaceholders(componentSections: Record<string, string>): Record<string, unknown> {
		...
		// Add commonly used template-level placeholders for custom user templates
		placeholders["agentName"] = "ShunCode AI"
		placeholders["userName"] = "User"
		placeholders["ide"] = "ShunCode"
		placeholders["memory"] = this.context.pinnedMemory || ""
		placeholders["currentTime"] = new Date().toLocaleTimeString()
		placeholders["mode"] = (this.context.mode || "act").toUpperCase() + " MODE"
		placeholders["workspace"] = this.context.cwd || process.cwd()
		placeholders["currentDateTime"] = new Date().toLocaleString()
		placeholders["provider"] = this.context.providerInfo?.providerId || ""
		placeholders["model"] = this.context.providerInfo?.model?.id || ""
		placeholders["pinnedMemory"] = this.context.pinnedMemory || ""
		placeholders["mcpSettingsPath"] = this.context.mcpSettingsPath || ""
		placeholders["gitStatus"] = this.context.gitStatus || {}
		placeholders["preferredLanguage"] = this.context.preferredLanguage || ""
		placeholders["osName"] = this.context.osName || ""
		placeholders["machineId"] = this.context.machineId || ""
	...
	private static getEnabledTools(variant: PromptVariant, context: SystemPromptContext) {
		let resolvedTools: ReturnType<typeof ShuncodeToolSet.getTools> = []
		...
		if (variant?.tools?.length) {
			...
			resolvedTools = ShuncodeToolSet.getToolsForVariantWithFallback(variant.family, requestedIds)
			...
			resolvedTools = ShuncodeToolSet.getTools(variant.family)
			// Sort by id for stable ordering
			resolvedTools = resolvedTools.sort((a, b) => a.config.id.localeCompare(b.config.id))
		}

		// Filter by context requirements and tool customization settings
		const enabledTools = resolvedTools
			.filter(
				(tool) => !tool.config.contextRequirements || tool.config.contextRequirements(context),
			)
			.filter((tool) => context.toolCustomizationSettings?.tools?.[tool.config.id]?.enabled !== false)
	...
	public static tool(config: ShuncodeToolSpec, registry: ShuncodeDefaultTool[], context: SystemPromptContext): string {
