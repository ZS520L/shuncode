...
export class Tensor {
  ...
  constructor(...args) {
    if (args[0] instanceof ONNXTensor) {
      // Create shallow copy from ORT Tensor.
      // [SHUNCODE] In onnxruntime-node >=1.20, `data` and `location` are prototype
      // getters, not own properties. Object.assign only copies own properties, so we
      // must explicitly read them via the getter and store as own properties.
      const src = args[0];
      Object.assign(this, src);
      if (this.data === undefined || this.data === null) {
        // `data` getter resolves cpuData; force-read it
        try { this.data = src.data ?? src.cpuData; } catch { /* ignore */ }
      }
    } else {
      // Create new tensor
      const newTensor = new ONNXTensor(
      ...
      Object.assign(this, newTensor);
      if (this.data === undefined || this.data === null) {
        try { this.data = newTensor.data ?? newTensor.cpuData; } catch { /* ignore */ }
      }
    }
