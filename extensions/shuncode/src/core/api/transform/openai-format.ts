 ...
 *
 * @param toolId - The original tool ID from Shuncode/Anthropic format
...
/**
 * Converts an array of ShuncodeStorageMessage objects to OpenAI's Completions API format.
 *
 * Handles conversion of Shuncode-specific content types (tool uses, tool results, images, reasoning details)
 * into OpenAI's expected message structure, including tool_calls and tool_call_id fields.
 *
 * @param anthropicMessages - Array of ShuncodeStorageMessage objects to be converted
 * @returns Array of OpenAI.Chat.ChatCompletionMessageParam objects
 */
export function convertToOpenAiMessages(
	anthropicMessages: Omit<ShuncodeStorageMessage, "modelInfo">[],
         ...
         */
			if (anthropicMessage.role === "user") {
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					nonToolMessages: (ShuncodeTextContentBlock | ShuncodeImageContentBlock)[]
					toolMessages: ShuncodeUserToolResultContentBlock[]
				}>(
					(acc, part) => {
				...
				const toolResultImages: ShuncodeImageContentBlock[] = []
				...
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					...
					toolMessages: ShuncodeAssistantToolUseBlock[]
				}>(
					(acc, part) => {
...
const UNIQUE_ERROR_TOOL_NAME = "_shuncode_error_unknown_function_"
