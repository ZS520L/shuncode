...
import type { ShuncodeStorageMessage } from "@shared/messages/content"
 ...
 */
export function extractTokenUsageFromMessage(message: ShuncodeMessage | undefined): TokenUsage {
...
export async function writePreCompactContextFiles(
	taskId: string,
	currentContext: ShuncodeStorageMessage[],
...
export interface PreCompactHookParams {
	...
	apiConversationHistory: ShuncodeStorageMessage[]
	/** Current deleted range (if any) */
	conversationHistoryDeletedRange?: [number, number]
	/** Shuncode messages for extracting token usage */
...
export async function executePreCompactHookWithCleanup(params: PreCompactHookParams): Promise<PreCompactHookResult> {
	...
	try {
		...
		const previousApiReqIndex = findLastIndex(params.shuncodeMessages, (m) => m.say === "api_req_started")
		const previousRequest = previousApiReqIndex !== -1 ? params.shuncodeMessages[previousApiReqIndex] : undefined
		...
		if (preCompactResult.cancel === true) {
			...
			await params.messageStateHandler.saveShuncodeMessagesAndUpdateHistory()
