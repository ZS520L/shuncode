...
import { DiagnosticSeverity } from "@/shared/proto/index.shuncode"
...
export async function parseMentions(
	...
	let parsedText = text.replace(mentionRegexGlobal, (match, mention) => {
		...
		if (mention.startsWith("http")) {
			return `'${mention}' (see above for site content)`
			...
			if (workspaceHint) {
				return mentionPath.endsWith("/")
					? `'${workspaceHint}:${mentionPath}' (see above for folder content)`
					: `'${workspaceHint}:${mentionPath}' (see above for file content)`
			}
			return mentionPath.endsWith("/")
				? `'${mentionPath}' (see above for folder content)`
				: `'${mentionPath}' (see above for file content)`
		} else if (mention === "problems") {
			return `Workspace Problems (see above for diagnostics)`
		} else if (mention === "terminal") {
			return `Terminal Output (see above for output)`
		} else if (mention === "git-changes") {
			return `Working directory changes (see above for details)`
		} else if (/^[a-f0-9]{7,40}$/.test(mention)) {
			return `Git commit '${mention}' (see above for commit info)`
		}
		return match
	})

	// Collect context blocks separately — they will be placed BEFORE the user text
	// so the LLM sees context first, then the user's request (recency bias)
	let contextBlocks = ""
	...
	for (const mention of uniqueMentions) {
		...
		// and enforce strict size/.shuncodeignore limits instead.
		if (mention === "/") {
			continue
		}
	...
	// Context blocks go BEFORE user text: LLM sees background first, user request last
	if (contextBlocks) {
