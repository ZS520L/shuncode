...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsShuncoderulesFileCheck: boolean }> {
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsShuncoderulesFileCheck: commandName === "newrule" }
			}
			...
			if (matchingWorkflow) {
				try {
					...
					return { processedText, needsShuncoderulesFileCheck: false }
				} catch (error) {
	...
	return { processedText: text, needsShuncoderulesFileCheck: false }
}
