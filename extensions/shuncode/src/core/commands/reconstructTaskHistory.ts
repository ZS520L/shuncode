import { getSavedShuncodeMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const shuncodeMessages = await getSavedShuncodeMessages(taskId)
		if (shuncodeMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(shuncodeMessages, metadata)
...
function extractTaskInformation(shuncodeMessages: ShuncodeMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = shuncodeMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = shuncodeMessages.length > 0 ? shuncodeMessages[0].ts : Date.now()
	...
	const apiReqMessages = shuncodeMessages.filter((msg) => msg.type === "say" && msg.say === "api_req_started" && msg.text)
	...
	const messageSize = JSON.stringify(shuncodeMessages).length
