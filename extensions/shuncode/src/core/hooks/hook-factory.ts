...
import { version as shuncodeVersion } from "../../../package.json"
...
} from "../../shared/proto/shuncode/hooks"
 ...
 * provide--the other common parameters like shuncodeVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - shuncodeVersion: Current Shuncode extension version
	 ...
	 * - userId: Shuncode user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: Shuncode User ID, machine ID, or generated UUID
			...params,
		}
	}
}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Shuncode/Hooks/)
 * and workspace-specific hooks (from each workspace's .shuncoderules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .shuncoderules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "Shuncode/Hooks" or "shuncode/hooks".
	 */
	 ...
	 * Global hooks are located in ~/Documents/Shuncode/Hooks/
	 * Workspace hooks are located in workspace .shuncoderules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/Shuncode/Hooks/) and workspace hooks
	 * (from .shuncoderules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .shuncoderules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .shuncoderules directory path to search
