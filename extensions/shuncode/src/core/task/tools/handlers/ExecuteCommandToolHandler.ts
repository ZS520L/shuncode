...
export class ExecuteCommandToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// [P4] Input validation pipeline (low-cost checks first)
		// 1. Block 'cd' as standalone command (model should use cwd parameter instead)
		if (/^\s*cd\s/i.test(command)) {
			return formatResponse.toolError(
				"Do not use 'cd' as a command. Instead, specify the target directory using the working directory context. " +
				"The command will be executed in the correct directory automatically."
			);
		}

		// 2. Block redundant 'open/code' commands — these files are already visible in the editor
		...
		if (openCommands.some(cmd => trimmedCommand.startsWith(cmd))) {
			...
			return formatResponse.toolError(
				"Commands like 'open', 'code', 'notepad' are not supported — files are already visible in the editor. " +
				"Use read_file to view file contents, or list_files to browse directories."
			);
		}
