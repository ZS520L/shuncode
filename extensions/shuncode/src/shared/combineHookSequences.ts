import { ShuncodeMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the ShuncodeSayHook interface but represents parsed data.
 */
...
function getSay(msg: ShuncodeMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: ShuncodeMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: ShuncodeMessage): HookMetadata | null {
 ...
 */
function filterPartialToolMessages(messages: ShuncodeMessage[]): ShuncodeMessage[] {
...
function combineHookWithOutputs(
	hookMessage: ShuncodeMessage,
	startIndex: number,
	messages: ShuncodeMessage[],
): { combined: ShuncodeMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: ShuncodeMessage[]): ShuncodeMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, ShuncodeMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: ShuncodeMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: ShuncodeMessage[], originalMessages: ShuncodeMessage[]): Map<number, ShuncodeMessage[]> {
	const map = new Map<number, ShuncodeMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: ShuncodeMessage[], preToolUseMap: Map<number, ShuncodeMessage[]>): ShuncodeMessage[] {
	const result: ShuncodeMessage[] = []
 ...
 *
 * @param messages Array of ShuncodeMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: ShuncodeMessage[]): ShuncodeMessage[] {
