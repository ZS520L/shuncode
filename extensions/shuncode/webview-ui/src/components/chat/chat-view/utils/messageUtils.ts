...
export function isReadFileTool(msg: ShuncodeMessage): boolean {
	...
	try {
		const tool = JSON.parse(msg.text || "{}") as { tool?: string }
		return tool.tool === "readFile"
	} catch {
		return false
	}
}

/** Get the filename (without path directories) from a readFile message for grouping */
function getReadFileBaseName(msg: ShuncodeMessage): string | null {
	try {
		const tool = JSON.parse(msg.text || "{}") as { tool?: string; path?: string }
		if (tool.tool !== "readFile" || !tool.path) return null
...
/**
 * Group consecutive readFile standalone messages for the same file into collapsible groups.
...
export function groupConsecutiveReadFiles(items: (ShuncodeMessage | ShuncodeMessage[])[]): (ShuncodeMessage | ShuncodeMessage[])[] {
	...
	let readBufferFileName: string | null = null
	...
	const commitReadBuffer = () => {
		...
		if (readBuffer.length === 1) {
			...
			// Multiple reads of same file — group them
			const group = readBuffer as ShuncodeMessage[] & { _isReadFileGroup: boolean }
			group._isReadFileGroup = true
			result.push(group)
		}
		readBuffer = []
		readBufferFileName = null
	}

	for (const item of items) {
		if (!Array.isArray(item) && isReadFileTool(item)) {
			const baseName = getReadFileBaseName(item)
			if (baseName && baseName === readBufferFileName) {
				// Same file — add to buffer
				readBuffer.push(item)
			} else {
				// Different file — commit previous and start new buffer
				commitReadBuffer()
				readBuffer = [item]
				readBufferFileName = baseName
			}
		} else {
