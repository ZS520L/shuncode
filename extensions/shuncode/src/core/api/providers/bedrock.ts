...
export class AwsBedrockHandler implements ApiHandler {
	...
	private async getAwsCredentials(): Promise<{
		...
		const providerOptions: ProviderChainOptions = {
			clientConfig: {
				...
				userAgentAppId: `shuncode#${ExtensionRegistryInfo.version}`,
			},
		}
		const useProfile =
	 ...
	 */
	private formatDeepseekR1Prompt(systemPrompt: string, messages: ShuncodeStorageMessage[]): string {
	 ...
	 */
	private estimateInputTokens(systemPrompt: string, messages: ShuncodeStorageMessage[]): number {
	 ...
	 */
	private formatMessagesForConverseAPI(messages: ShuncodeStorageMessage[]): Message[] {
