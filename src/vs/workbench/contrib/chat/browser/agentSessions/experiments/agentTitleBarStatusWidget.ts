...
export class AgentTitleBarStatusWidget extends BaseActionViewItem {
	...
	private _openShunCodeChat(): void {
		this.telemetryService.publicLog2<AgentStatusClickEvent, AgentStatusClickClassification>('agentStatusControl.click', {
			source: 'sparkle',
			action: 'openShunCodeChat'
		});
		this.commandService.executeCommand('shuncode.SidebarProvider.focus');
	}
	...
	private _renderStatusBadge(disposables: DisposableStore, activeSessions: IAgentSession[], unreadSessions: IAgentSession[], attentionNeededSessions: IAgentSession[]): void {
		...
		sparkleContainer.setAttribute('role', 'button');
		sparkleContainer.setAttribute('aria-label', localize('openShunCodeChat', "Open ShunCode Chat"));
		sparkleContainer.tabIndex = 0;
		if (!this._firstFocusableElement) {
			this._firstFocusableElement = sparkleContainer;
		}
		badge.appendChild(sparkleContainer);
		const sparkleIcon = $('span.agent-status-icon');
		reset(sparkleIcon, renderIcon(Codicon.chatSparkle));
		sparkleContainer.appendChild(sparkleIcon);
		disposables.add(addDisposableListener(sparkleContainer, EventType.CLICK, (e) => {
			e.preventDefault();
			e.stopPropagation();
			this._openShunCodeChat();
		}));
