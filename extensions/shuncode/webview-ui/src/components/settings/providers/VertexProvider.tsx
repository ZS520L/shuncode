		...
		<div
			...
			<RemotelyConfiguredInputWrapper hidden={remoteConfigSettings?.vertexProjectId === undefined}>
				<DebouncedTextField
					...
					placeholder={t("provider.enterProjectId")}
					style={{ width: "100%" }}>
					<div className="flex items-center gap-2 mb-1">
						<span style={{ fontWeight: 500 }}>{t("provider.googleCloudProjectId")}</span>
				...
				<DropdownContainer className="dropdown-container" zIndex={DROPDOWN_Z_INDEX - 1}>
					<div
						...
						<label htmlFor="vertex-region-dropdown">
							<span className="font-medium">{t("provider.googleCloudRegion")}</span>
						</label>
			...
			<p
				...
				{t("provider.vertexAiSetupPrefix")}
				<VSCodeLink
					...
					{selectedModelInfo.thinkingConfig?.supportsThinkingLevel && (
						<DropdownContainer className="dropdown-container" style={{ marginTop: "8px" }} zIndex={1}>
							<label htmlFor="thinking-level">
								<span style={{ fontWeight: 500 }}>{t("provider.thinkingLevel")}</span>
							</label>
							<VSCodeDropdown
								className="w-full"
								id="thinking-level"
								...
								<VSCodeOption value="low">{t("provider.low")}</VSCodeOption>
								<VSCodeOption value="high">{t("provider.high")}</VSCodeOption>
							</VSCodeDropdown>
						</DropdownContainer>
					)}
