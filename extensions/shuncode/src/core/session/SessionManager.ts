...
export class SessionManager {
	...
	create(id?: string): Session {
		...
		// Enforce max sessions limit: remove oldest idle session if at capacity
	...
	/**
	 * Evict the oldest idle/paused session to make room for a new one.
	 * Prefers sessions in 'idle' or 'done' state, then 'paused'.
	 * Never evicts the active session.
	 */
	private evictOldestIdle(): void {
		// Find candidates ordered by preference: idle/done > paused
		let candidate: string | null = null
		for (const [id, session] of this.sessions) {
			if (id === this._activeSessionId) continue
			if (session.state === "idle" || session.state === "done") {
				candidate = id
				break
			}
			if (session.state === "paused" && !candidate) {
				candidate = id
			}
		}
		if (candidate) {
