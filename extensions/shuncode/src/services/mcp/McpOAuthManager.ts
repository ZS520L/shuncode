...
/**
 * Implementation of OAuthClientProvider for Shuncode
 * Manages OAuth state and token storage for a single MCP server
 */
class ShuncodeOAuthClientProvider implements OAuthClientProvider {
	...
	get clientMetadata(): OAuthClientMetadata {
		return {
			...
			client_uri: "https://shuncode-ai.local", // TODO: заменить на свой домен
			software_id: "shuncode",
		}
	}
...
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		// Create provider
		const provider = new ShuncodeOAuthClientProvider(serverName, serverUrl)
