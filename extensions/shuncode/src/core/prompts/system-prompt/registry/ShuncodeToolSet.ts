...
/**
 * Tools that must be hidden in Plan mode (read + analyze only).
 * Plan mode agents can read, search, and analyze code but cannot write.
 * Inspired by OpenCode's approach: "Tools the LLM can't see, it can't call."
 */
const PLAN_MODE_HIDDEN_TOOLS: ReadonlySet<string> = new Set([
	ShuncodeDefaultTool.FILE_NEW,
	ShuncodeDefaultTool.FILE_EDIT,
	ShuncodeDefaultTool.NEW_RULE,
	ShuncodeDefaultTool.APPLY_PATCH,
...
export class ShuncodeToolSet {
	...
	public static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): ShuncodeToolSet[] {
		...
		for (const id of requestedIds) {
			...
			// In Plan mode, skip write tools — model can't see them so it can't call them
			if (context.mode === "plan" && PLAN_MODE_HIDDEN_TOOLS.has(id)) {
				continue
			}
