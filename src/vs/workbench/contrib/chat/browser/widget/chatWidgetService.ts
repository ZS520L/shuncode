...
import { ChatViewId, ChatViewContainerId, ChatViewPaneTarget, IChatWidget, IChatWidgetService, IQuickChatService, isIChatViewViewContext } from '../chat.js';
...
export class ChatWidgetService extends Disposable implements IChatWidgetService {
	...
	async revealWidget(preserveFocus?: boolean): Promise<IChatWidget | undefined> {
		...
		const result = (await this.viewsService.openView<ChatViewPane>(ChatViewId, !preserveFocus))?.widget;
		if (result) {
			return result;
		}

		// [SHUNCODE] Fallback: built-in chat view is disabled, open the first
		// available view in the chat container (e.g. ShunCode extension webview)
		const container = this.viewDescriptorService.getViewContainerById(ChatViewContainerId);
		if (container) {
			const views = this.viewDescriptorService.getViewContainerModel(container).visibleViewDescriptors;
			if (views.length > 0) {
				await this.viewsService.openView(views[0].id, !preserveFocus);
			}
		}
		return undefined;
	}
