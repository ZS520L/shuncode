...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's Shuncode telemetry opt-in/opt-out settings.
	 * This is used for:
	 * 	- User-controlled collectors configured via environment variables (e.g., SHUNCODE_OTEL_TELEMETRY_ENABLED).
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// If the user has `SHUNCODE_OTEL_TELEMETRY_ENABLED` in his environment, enable
				// OTEL regardless of his Shuncode telemetry settings
