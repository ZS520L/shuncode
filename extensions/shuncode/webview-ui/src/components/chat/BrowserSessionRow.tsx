...
import { BrowserAction, BrowserActionResult, ShuncodeMessage, ShuncodeSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: ShuncodeMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: ShuncodeMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: ShuncodeMessage[] = []
		let nextActionMessages: ShuncodeMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? t("chat.shuncodeUsingBrowser") : t("chat.shuncodeWantsToUseBrowser")}
				</span>
			</div>
			<div
				style={{
				...
				<div style={consoleLogsContainerStyle}>
					<button
						...
						className="group flex w-full items-center gap-1.5 bg-transparent border-0 text-description text-left py-2 px-2 cursor-pointer select-none transition-all duration-150 hover:bg-secondary/25 hover:text-foreground active:bg-secondary/45 active:scale-[0.997]"
						onClick={() => {
							setConsoleLogsExpanded(!consoleLogsExpanded)
						}}
						type="button">
						<ChevronRightIcon className={cn("size-3 shrink-0 transition-transform duration-150", { "rotate-90": consoleLogsExpanded })} />
						<span className="font-medium" style={consoleLogsTextStyle}>{t("chat.consoleLogs")}</span>
						<div className="grow" />
						<span className="text-[11px] text-description/70 transition-colors group-hover:text-foreground">
							{consoleLogsExpanded ? "Collapse" : "Expand"}
						</span>
					</button>
					{consoleLogsExpanded && (
...
const BrowserActionBox = ({ action, coordinate, text }: { action: BrowserAction; coordinate?: string; text?: string }) => {
	...
	return (
		<div style={browserActionBoxContainerStyle}>
			<div className="overflow-hidden rounded-md border border-editor-group-border bg-code transition-colors duration-150">
				<div className="flex items-center py-2 px-2.5 text-description">
