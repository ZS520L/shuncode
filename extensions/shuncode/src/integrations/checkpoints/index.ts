...
import { ShuncodeApiReqInfo, ShuncodeMessage, ShuncodeSay } from "@shared/ExtensionMessage"
import { getApiMetrics } from "@shared/getApiMetrics"
import { HistoryItem } from "@shared/HistoryItem"
import { ShuncodeCheckpointRestore } from "@shared/WebviewMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const shuncodeMessages = this.services.messageStateHandler.getShuncodeMessages()
			shuncodeMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getShuncodeMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveShuncodeMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								Logger.error(
				...
				const lastFiveshuncodeMessages = this.services.messageStateHandler.getShuncodeMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFiveshuncodeMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: ShuncodeCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const shuncodeMessages = this.services.messageStateHandler.getShuncodeMessages()
			const messageIndex = shuncodeMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(shuncodeMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = shuncodeMessages[messageIndex]
			const lastMessageWithHash = shuncodeMessages[lastHashIndex]
		...
		try {
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getShuncodeMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const shuncodeMessages = this.services.messageStateHandler.getShuncodeMessages()
			const messageIndex = findLastIndex(shuncodeMessages, (m) => m.say === "completion_result")
			const message = shuncodeMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const shuncodeMessages = this.services.messageStateHandler.getShuncodeMessages()
				const deletedMessages = shuncodeMessages.slice(messageIndex + 1)
				...
				const newShuncodeMessages = shuncodeMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteShuncodeMessages(newShuncodeMessages) // calls saveShuncodeMessages which saves historyItem
