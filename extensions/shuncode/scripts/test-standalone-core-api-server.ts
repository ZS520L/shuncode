...
/**
 * Simple Shuncode gRPC Server
 *
 * This script provides a minimal way to run the Shuncode core gRPC service
 ...
 *   2. ShuncodeApiServerMock (mock implementation of the Shuncode API)
 ...
 *   SHUNCODE_DIST_DIR - Override distribution directory (default: PROJECT_ROOT/dist-standalone)
 *   SHUNCODE_CORE_FILE - Override core file name (default: shuncode-core.js)
 ...
 *   SHUNCODE_ENVIRONMENT - Environment setting (default: local)
 *
...
import { ShuncodeApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const SHUNCODE_ENVIRONMENT = process.env.SHUNCODE_ENVIRONMENT || "local"
...
const distDir = process.env.SHUNCODE_DIST_DIR || path.join(projectRoot, "dist-standalone")
const shuncodeCoreFile = process.env.SHUNCODE_CORE_FILE || "shuncode-core.js"
const coreFile = path.join(distDir, shuncodeCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple Shuncode gRPC Server...")
	...
	if (!fs.existsSync(coreFile)) {
		...
		console.error("  SHUNCODE_DIST_DIR - Override distribution directory")
		console.error("  SHUNCODE_CORE_FILE - Override core file name")
		console.error("")
	...
	try {
		await ShuncodeApiServerMock.startGlobalServer()
		console.log("Shuncode API Server started in-process")
	} catch (error) {
		console.error("Failed to start Shuncode API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const shuncodeTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "shuncode-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("npx", ["tsx", path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: shuncodeTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", path.join(distDir, "shuncode-core.js")]

	const spawnArgs = USE_C8 ? ["c8", "--report-dir", covDir, "node", ...baseArgs] : ["node", ...baseArgs]

	console.log(`Starting Shuncode Core Service... (useC8=${USE_C8})`)
	...
	const coreService: ChildProcess = spawn("npx", spawnArgs, {
		...
		env: {
			...
			E2E_TEST,
			SHUNCODE_ENVIRONMENT,
			SHUNCODE_DIR: userDataDir,
	...
	const shutdown = async () => {
		...
		await ShuncodeApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(shuncodeTestWorkspace, { recursive: true, force: true })
	...
	console.log(`Shuncode gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple Shuncode server:", err)
		process.exit(1)
	})
}
