...
const getShuncodeVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const shuncodeMatch = extensions.match(/shuncode@(\d+\.\d+\.\d+)/)
		return shuncodeMatch ? shuncodeMatch[1] : "Not installed"
	} catch (_err) {
...
const collectSystemInfo = () => {
	...
	return {
		...
		shuncodeVersion: getShuncodeVersion(),
	}
}
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/shuncode/shuncode/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&shuncode-version=${systemInfo.shuncodeVersion}` +
		`&system-info=${
