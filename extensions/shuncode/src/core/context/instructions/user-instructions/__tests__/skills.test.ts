...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".shuncode", "skills")
...
Instructions here`)
		...
		it("should discover skills from project .shuncoderules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".shuncoderules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .shuncode/skills directory", async () => {
			const shuncodeSkillsDir = path.join(TEST_CWD, ".shuncode", "skills")
			const skillDir = path.join(shuncodeSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(shuncodeSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(shuncodeSkillsDir).resolves(true)
			readdirStub.withArgs(shuncodeSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
