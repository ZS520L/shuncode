 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "shuncode"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "shuncode"
...
async function generateShuncodeClientPy(outDir, services) {
...
class ShuncodeClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/ShuncodeClient.

    Usage:
        client = ShuncodeClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "shuncode_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// shuncode_client.py (unified that composes service wrappers)
	await generateShuncodeClientPy(clientDir, services)
}
...
[project]
name = "shuncode-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for Shuncode protos"
