...
vi.mock("@/context/ShuncodeAuthContext", () => ({
	useShuncodeAuth: () => ({
		shuncodeUser: null,
	}),
	useShuncodeSignIn: () => ({
...
// Mock ShuncodeError
vi.mock("../../../../src/services/error/ShuncodeError", () => ({
	ShuncodeError: {
		parse: vi.fn(),
	},
	ShuncodeErrorType: {
...
describe("ErrorRow", () => {
	const mockMessage: ShuncodeMessage = {
		ts: 123456789,
		type: "say",
		say: "error",
	...
	it("renders shuncodeignore error", () => {
		const shuncodeignoreMessage = { ...mockMessage, text: "/path/to/file.txt" }
		render(<ErrorRow errorType="shuncodeignore_error" message={shuncodeignoreMessage} />)

		expect(screen.getByText(/Shuncode tried to access/)).toBeInTheDocument()
		expect(screen.getByText("/path/to/file.txt")).toBeInTheDocument()
	})

	describe("API error handling", () => {
		it("renders credit limit error when balance error is detected", async () => {
			const mockShuncodeError = {
			...
			const { ShuncodeError } = await import("../../../../src/services/error/ShuncodeError")
			vi.mocked(ShuncodeError.parse).mockReturnValue(mockShuncodeError as any)
		...
		it("renders auth error with sign in button when user is not signed in", async () => {
			...
			expect(screen.getByText("Sign in to Shuncode")).toBeInTheDocument()
		})
		...
		it("falls back to regular error message when ShuncodeError.parse returns null", async () => {
			const { ShuncodeError } = await import("../../../../src/services/error/ShuncodeError")
			vi.mocked(ShuncodeError.parse).mockReturnValue(undefined)

			render(<ErrorRow apiRequestFailedMessage="Some API error" errorType="error" message={mockMessage} />)

			// When ShuncodeError.parse returns null, we display the raw error message for non-Shuncode providers
			// Since shuncodeError is undefined, isShuncodeProvider is false, so we show the raw apiRequestFailedMessage
