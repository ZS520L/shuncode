...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from ShuncodeMessages?
		const shuncodeMessages = currentTask.messageStateHandler.getShuncodeMessages()
		const lastProgressMessage = shuncodeMessages
			.slice()
			.reverse()
