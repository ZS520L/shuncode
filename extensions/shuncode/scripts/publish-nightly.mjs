 ...
 *    - Changes name to "shuncode-nightly"
 *    - Changes displayName to "Shuncode (Nightly)"
...
const config = {
	...
	nightlyName: "shuncode-nightly",
	nightlyDisplayName: "Shuncode (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "shuncode-nightly.vsix")
	},
}
...
class NightlyPublisher {
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences of shuncode with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll('"shuncode.', `"${config.nightlyName}.`)
