 ...
 * Designed to be instantiated once in Shuncode.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .shuncodeignore files.
 */
export class ShuncodeIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	shuncodeIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.shuncodeIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .shuncodeignore
		this.setupFileWatcher()
		await this.loadShuncodeIgnore()
	}

	/**
	 * Set up the file watcher for .shuncodeignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".shuncodeignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .shuncodeignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .shuncodeignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadShuncodeIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.shuncodeIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".shuncodeignore")
			} else {
				this.shuncodeIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .shuncodeignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processShuncodeIgnoreIncludes(content)
	 ...
	 */
	private async processShuncodeIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[ShuncodeIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .shuncodeignore does not exist
		if (!this.shuncodeIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .shuncodeignore exists
		if (!this.shuncodeIgnoreContent) {
			return undefined
		}
