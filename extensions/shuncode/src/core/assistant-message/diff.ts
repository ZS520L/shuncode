...
/**
 * Fourth-tier fallback: Indentation-normalized matching.
 * Normalizes tabs to spaces and compares content ignoring indentation width differences.
 * Handles cases where AI uses 2-space indent but file uses tabs, or vice versa.
 */
function indentNormalizedFallbackMatch(originalContent: string, searchContent: string, startIndex: number): [number, number] | false {
	const normalize = (line: string) => line.replace(/^[\t ]+/, (ws) => ws.replace(/\t/g, "    ")).trimEnd()
...
/**
 * Fifth-tier fallback: Line-level similarity matching.
 * Uses a simple Jaccard-like similarity to find the best matching block in the file.
 * Only returns if similarity >= 0.85 (85%) to avoid false positives.
 * Inspired by Aider's replace_most_similar_chunk.
 */
function similarityFallbackMatch(originalContent: string, searchContent: string, startIndex: number): [number, number] | false {
	const originalLines = originalContent.split("\n")
	const searchLines = searchContent.split("\n")

	if (searchLines[searchLines.length - 1] === "") searchLines.pop()
	if (searchLines.length < 2) return false // Too short for similarity matching
	...
	for (let i = startLineNum; i <= originalLines.length - searchLines.length; i++) {
		let matchingLines = 0
		for (let j = 0; j < searchLines.length; j++) {
			const origTrimmed = originalLines[i + j].trim()
			const searchTrimmed = searchLines[j].trim()
			if (origTrimmed === searchTrimmed) {
				matchingLines++
			} else if (origTrimmed.length > 0 && searchTrimmed.length > 0) {
				// Partial line similarity: check if significant tokens overlap
				const origTokens = new Set(origTrimmed.split(/\s+/))
				const searchTokens = searchTrimmed.split(/\s+/)
				const overlap = searchTokens.filter((t) => origTokens.has(t)).length
				if (overlap / Math.max(searchTokens.length, 1) > 0.7) {
					matchingLines += 0.7
				}
			}
		}

		const score = matchingLines / searchLines.length
		if (score > bestScore) {
			bestScore = score
			bestStart = i
		}
	...
	if (bestScore >= SIMILARITY_THRESHOLD && bestStart >= 0) {
		let matchStartIndex = 0
		for (let k = 0; k < bestStart; k++) matchStartIndex += originalLines[k].length + 1
		let matchEndIndex = matchStartIndex
		for (let k = 0; k < searchLines.length; k++) matchEndIndex += originalLines[bestStart + k].length + 1
		return [matchStartIndex, matchEndIndex]
	}
...
/**
 * Diagnostic helper: finds the most similar block in the file to a failed SEARCH content.
 * Used to provide actionable feedback when all matching strategies fail.
 * Returns the best-matching lines with their location and similarity percentage.
 */
function findMostSimilarLines(
	originalContent: string,
	searchContent: string,
): { startLine: number; endLine: number; similarity: number; preview: string } | null {
	const originalLines = originalContent.split("\n")
	const searchLines = searchContent.split("\n")

	if (searchLines[searchLines.length - 1] === "") searchLines.pop()
	if (searchLines.length === 0 || originalLines.length === 0) return null
	...
	// Only report if we found something at least 40% similar
	if (bestScore < 0.4 || bestStart < 0) return null

	const previewLines = originalLines.slice(bestStart, bestStart + searchLines.length)
...
export async function constructNewFileContent(
	...
	isFinal: boolean,
	version: "v1" | "v2" = "v1",
): Promise<{ newContent: string; matchIndices: number[]; replacements?: Array<{ start: number; end: number; originalContent: string; content: string }> }> {
...
const constructNewFileContentVersionMapping: Record<
	string,
	(diffContent: string, originalContent: string, isFinal: boolean) => Promise<{ newContent: string; matchIndices: number[]; replacements?: Array<{ start: number; end: number; originalContent: string; content: string }> }>
> = {
...
async function constructNewFileContentV1(
	...
	const replacements: Array<{ start: number; end: number; originalContent: string; content: string }> = []
	...
	for (const line of lines) {
				...
				if (exactIndex !== -1) {
					...
					if (lineMatch) {
						...
						if (blockMatch) {
							...
							// Try indentation-normalized matching (tabs vs spaces)
							const indentMatch = indentNormalizedFallbackMatch(originalContent, currentSearchContent, lastProcessedIndex)
							if (indentMatch) {
								;[searchMatchIndex, searchEndIndex] = indentMatch
							} else {
								// Try similarity-based matching (85%+ line similarity)
								const simMatch = similarityFallbackMatch(originalContent, currentSearchContent, lastProcessedIndex)
								if (simMatch) {
									;[searchMatchIndex, searchEndIndex] = simMatch
								} else {
									...
									if (fullFileIndex !== -1) {
										...
										// All matching strategies exhausted — provide diagnostic
										const suggestion = findMostSimilarLines(originalContent, currentSearchContent)
										const diagMessage = suggestion
											? `\n\nMost similar content found (${suggestion.similarity}% match) at lines ${suggestion.startLine}-${suggestion.endLine}:\n${suggestion.preview}\n\nDid you mean this block? Check for whitespace, indentation, or content differences.`
											: ""
										throw new Error(
											`The SEARCH block:\n${currentSearchContent.trimEnd()}\n...does not match anything in the file.${diagMessage}`,
										)
									}
								}
							}
		...
		if (isReplaceBlockEnd(line)) {
			...
			if (searchMatchIndex === -1) {
				throw new Error(
					`The SEARCH block:\n${currentSearchContent.trimEnd()}\n...is malformatted. Did you forget the '=======' separator?`,
				)
			}
			...
			replacements.push({
				...
				originalContent: originalContent.slice(searchMatchIndex, searchEndIndex),
	...
	return { newContent: result, matchIndices: replacements.map((r) => r.start), replacements }
}
