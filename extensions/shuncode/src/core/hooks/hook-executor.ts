...
import type { HookOutput } from "@shared/proto/shuncode/hooks"
...
async function updateHookMessage(
	...
	const hookMessageIndex = shuncodeMessages.findIndex((m: ShuncodeMessage) => m.ts === hookMessageTs)
	if (hookMessageIndex !== -1) {
		await messageStateHandler.updateShuncodeMessage(hookMessageIndex, {
...
async function reorderHookAndToolMessages(messageStateHandler: MessageStateHandler): Promise<void> {
	...
	for (let i = shuncodeMessages.length - 1; i >= 0; i--) {
		const msgType = shuncodeMessages[i].ask || shuncodeMessages[i].say
	...
	for (let i = lastToolMessageIndex + 1; i < shuncodeMessages.length; i++) {
		if (shuncodeMessages[i].say === "hook_status" || shuncodeMessages[i].say === "hook_output_stream") {
	...
	const toolMessage = { ...shuncodeMessages[lastToolMessageIndex] }

	// Delete the tool message at its current position
	await messageStateHandler.deleteShuncodeMessage(lastToolMessageIndex)

	// Re-add the tool message at the end (after hook messages)
	await messageStateHandler.addToShuncodeMessages(toolMessage)
}
