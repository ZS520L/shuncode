...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/ShuncodeAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { ShuncodeDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_SHUNCODE_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.SHUNCODE_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[ShuncodeApiServerMock]", ...args)
	}
}

export class ShuncodeApiServerMock {
	static globalSharedServer: ShuncodeApiServerMock | null = null
	...
	public readonly API_USER = new ShuncodeDataMock("personal")
	...
	public static async startGlobalServer(): Promise<ShuncodeApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (ShuncodeApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return ShuncodeApiServerMock.globalSharedServer
		}
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (isAuthRequired && authToken) {
				...
				const user = ShuncodeApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				ShuncodeApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = ShuncodeApiServerMock.matchRoute(path, method)
				...
				const controller = ShuncodeApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching ShuncodeAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Shuncode!")) {
								...
								choices: [
									{
										...
										message: {
											role: "assistant",
											content: "Hello! I'm a mock Shuncode API response.",
										},
										finish_reason: "stop",
									},
								],
		...
		const controller = new ShuncodeApiServerMock(server)
		ShuncodeApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			ShuncodeApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				ShuncodeApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`ShuncodeApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = ShuncodeApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		ShuncodeApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		ShuncodeApiServerMock.globalSockets.clear()
		...
		ShuncodeApiServerMock.globalSharedServer = null
	}
}
