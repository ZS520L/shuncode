...
export class ToolValidator {
	constructor(private readonly shuncodeIgnoreController: ShuncodeIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .shuncodeignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkShuncodeIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.shuncodeIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .shuncodeignore settings.`,
			}
		}
		return { ok: true }
	}
