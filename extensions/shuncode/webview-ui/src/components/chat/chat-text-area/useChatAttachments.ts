...
export function useChatAttachments({
	...
	// --- SHUNCODE: Listen for file drop forwarded from the workbench overlay ---
	// When the webview's pointer-events are blocked during drag, the WebviewViewPane
	// captures the DROP event and forwards file URIs via postMessage.
	useEffect(() => {
		const handleFileDropMessage = (event: MessageEvent) => {
			const msg = event.data
			if (!msg || msg.type !== "shuncodeFileDrop" || !Array.isArray(msg.uris)) {
				return
			}

			const validUris = (msg.uris as string[]).filter(
				(uri) =>
					uri &&
			...
			if (validUris.length > 0) {
				...
				FileServiceClient.getRelativePaths(RelativePathsRequest.create({ uris: validUris }))
					...
					.catch((error) => {
						console.error("Error getting relative paths from workbench drop:", error)
					})
			}
		}

		window.addEventListener("message", handleFileDropMessage)
		return () => window.removeEventListener("message", handleFileDropMessage)
	}, [inputValue, setPendingInsertions, setIntendedCursorPosition, textAreaRef])

	// --- SHUNCODE: Listen for terminal drop forwarded from the workbench overlay ---
	// When a terminal tab/instance is dragged into the ShunCode panel, insert @terminal
	// which will be resolved by the mention system to include the latest terminal output.
	useEffect(() => {
		const handleTerminalDropMessage = (event: MessageEvent) => {
			const msg = event.data
			if (!msg || msg.type !== "shuncodeTerminalDrop") {
				return
			}
			...
			setIntendedCursorPosition(initialCursorPos)
			setPendingInsertions(["terminal"])
		}

		window.addEventListener("message", handleTerminalDropMessage)
		return () => window.removeEventListener("message", handleTerminalDropMessage)
	}, [inputValue, setPendingInsertions, setIntendedCursorPosition, textAreaRef])
