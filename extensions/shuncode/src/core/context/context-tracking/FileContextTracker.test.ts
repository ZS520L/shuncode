...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.shuncode_read_date).to.be.a("number")
		expect(fileEntry.shuncode_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Shuncode", async () => {
		await tracker.trackFileContext(filePath, "shuncode_edited")
		...
		expect(activeEntry.record_source).to.equal("shuncode_edited")
		expect(activeEntry.shuncode_read_date).to.be.a("number")
		expect(activeEntry.shuncode_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				shuncode_read_date: Date.now() - 1000, // 1 second ago
				shuncode_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("shuncode_edited")
	})
	...
	it("should not track Shuncode edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Shuncode
		tracker.markFileAsEditedByShuncode(filePath)
