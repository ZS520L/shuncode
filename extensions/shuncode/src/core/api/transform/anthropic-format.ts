...
import { ShuncodeStorageMessage, convertShuncodeStorageToAnthropicMessage } from "@/shared/messages/content"

/**
 * Converts Shuncode storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param shuncodeMessages - Array of Shuncode storage messages to convert
...
export function sanitizeAnthropicMessages(
	shuncodeMessages: Array<ShuncodeStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = shuncodeMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return shuncodeMessages.map((msg, index) => {
		const anthropicMsg = convertShuncodeStorageToAnthropicMessage(msg)
