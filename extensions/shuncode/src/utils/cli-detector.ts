...
/**
 * Check if the Shuncode CLI tool is installed on the system
 * @returns true if CLI is installed, false otherwise
 */
export async function isShuncodeCliInstalled(): Promise<boolean> {
	try {
		// Try to get the version of the shuncode CLI tool
		// This will fail if the tool is not installed
		const { stdout } = await execAsync("shuncode version", {
			timeout: 5000, // 5 second timeout
		})

		// If we get here, the CLI is installed
		// We could also validate the version if needed
		return stdout.includes("Shuncode CLI Version") || stdout.includes("Shuncode Core Version")
	} catch (error) {
...
/**
 * Detect if the current Shuncode instance is running as a CLI subagent.
 * CLI subagents are identified by specific parameter patterns set by the transformShuncodeCommand function.
 *     TODO - For now we are relying on the maxConsecutiveMistakes value, which will only ever be "3"
 *     unless users pass in "-s max_consecutive_mistakes=6" via Shuncode CLI. Would like better detection.
