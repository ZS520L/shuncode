...
import { ApiFormat } from "@/shared/proto/index.shuncode"
...
export class OcaHandler implements ApiHandler {
	...
	async *createMessageChatApi(systemPrompt: string, messages: ShuncodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureClient()
		...
		const chatCompletionsParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
			...
			...(this.options.taskId && {
				litellm_session_id: `shuncode-${this.options.taskId}`,
	...
	async *createMessageResponsesApi(systemPrompt: string, messages: ShuncodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureClient()
