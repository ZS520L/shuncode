...
export abstract class WebviewProvider {
	...
	public updateSessionTabs(_tabs: Array<{ id: string; title: string; state?: string }>, _currentId?: string): void {
		// No-op in base. Overridden in VscodeWebviewProvider to update native title tabs.
	}
	...
	protected async getHMRHtmlContent(): Promise<string> {
		...
		try {
			...
			// Fall back silently to bundled assets when the local dev server is unavailable.
			if (process.env.IS_DEV) {
				Logger.warn(
					"ShunCode: Local webview dev server is not running, HMR will not work. Using bundled assets.",
				)
			}
