...
import { TerminalInteractiveResponder, type InteractiveResponderConfig } from "../TerminalInteractiveResponder"
...
export class StandaloneTerminalProcess extends EventEmitter<TerminalProcessEvents> implements ITerminalProcess {
	...
	/** Interactive responder for auto-responding to terminal prompts */
	private interactiveResponder: TerminalInteractiveResponder | null = null

	/** Timer for detecting prompts in incomplete buffer (no trailing newline) */
	private promptDetectionTimer: NodeJS.Timeout | null = null

	/** Delay before checking buffer for prompts (ms) */
	private static readonly PROMPT_DETECTION_DELAY_MS = 1500

	constructor(interactiveResponderConfig?: InteractiveResponderConfig) {
		super()
		if (interactiveResponderConfig?.enabled) {
			this.interactiveResponder = new TerminalInteractiveResponder(interactiveResponderConfig)
		}
	}
	...
	async run(terminal: ITerminal, command: string): Promise<void> {
		...
		try {
			// Enable stdin pipe when interactive responder is active, otherwise ignore stdin
			const stdinMode = this.interactiveResponder ? "pipe" as const : "ignore" as const
			...
			const shellOptions: {
				...
				cwd: cwd,
				stdio: [stdinMode, "pipe", "pipe"], // Enable STDIN when interactive responder is active
				env: {
					...process.env,
					TERM: "xterm-256color",
			...
			this.childProcess.stdout?.on("data", (data: Buffer) => {
				...
				this.schedulePromptDetection(output)
			})
	...
	/**
	 * Schedule prompt detection on the current buffer.
	 * Prompts typically don't end with a newline, so they remain in the buffer.
	 * We wait a short delay after the last output to check if the buffer looks like a prompt.
	 *
	 * @param _latestOutput The latest output chunk (used for context)
	 */
	private schedulePromptDetection(_latestOutput: string): void {
		if (!this.interactiveResponder || !this.childProcess?.stdin) {
			return
		}
		...
		// Wait for output to settle, then check buffer for prompts
		this.promptDetectionTimer = setTimeout(() => {
			if (!this.buffer || !this.interactiveResponder || !this.childProcess?.stdin) {
				return
			}

			const result = this.interactiveResponder.detectPromptInBuffer(this.buffer)
			if (result.detected && result.response) {
				try {
					this.childProcess.stdin.write(result.response + "\n")
					Logger.info(`[StandaloneTerminalProcess] Auto-responded "${result.response}" to prompt (${result.matchedPattern})`)
				} catch (error) {
					Logger.error(`[StandaloneTerminalProcess] Failed to write to stdin:`, error)
				}
			}
		}, StandaloneTerminalProcess.PROMPT_DETECTION_DELAY_MS)
	}
	...
	getUnretrievedOutput(): string {
		...
		this.lastRetrievedIndex = this.fullOutput.length
		return this.truncateOutput(unretrieved, MAX_UNRETRIEVED_LINES)
	}

	getOutput(lineLimit?: number): string {
		return this.truncateOutput(this.fullOutput, lineLimit)
	}

	private truncateOutput(output: string, lineLimit: number = MAX_UNRETRIEVED_LINES): string {
		const lines = output.split("\n")
		if (lines.length > lineLimit) {
			const keepLines = Math.min(TRUNCATE_KEEP_LINES, Math.floor(lineLimit / 2))
